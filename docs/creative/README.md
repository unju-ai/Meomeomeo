...
`src/shared/BrandCatalog.luau` is the runtime source for the trio's identities, colors, taglines and mock lines. `NpcCatalog` registers the hosts, `MockAiProvider` uses their authored lines, and `MapBuilder` places three non-colliding prototype figures by the lobby. The existing NPC chat prompt opens the conversation. The five-mode hub also has a sixth **Meet the cats** card: MEO, ME and MO each have a portrait button using their shared colors, blank stare and cobalt braincell. These buttons open the same authored greeting and conversation without requiring players to walk behind the hub panel. The trio remains a cast entry, not a sixth game mode. Conversations close when gameplay begins. Existing shopkeepers, coach and announcer retain their roles.
