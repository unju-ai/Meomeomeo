...
- **Combat extras:** shield absorb and a short WalkSpeed stun stub (server-authoritative). Original-six identity lives in `Shared.ChampionKits` and is applied only from `CombatService`: Meow guard/pull, Nyan takedown reset and breakable channel, Whiskers mana refund and ticking zone, Chonk Settled Loaf and Charge Knock, Scammy Open Wick and Hype Candle, Grandma Second Helping and Sweater Aura. The eight expansion cats stay on the zero plan.
