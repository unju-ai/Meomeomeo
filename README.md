...
- **Vision / fog of war:** server builds a visibility set every ~0.25s (`VisionUpdated` + packed `fogBits`). Living **ally champions**, **minions**, **towers/nexus**, **trinkets**, and **pink wards** each grant a radius bubble (`Config.Vision`). **Base walls** block vision rays (mid-lane gate stays open; jungle brush is not a wall). The client darkens ground outside that mask (10-stud grid) and hides unseen enemies. Dead cats do not grant vision. You cannot AA or attack-move onto a fogged / in-brush target. Enemy traveling bolts hide in unseen fog; your own stay visible.
...
3. Fog polish is in (wall LoS, skillshot FX cull, brush AA). Next: reconnect fog memory, mesh LoS, brush attack-move pathing into the pocket.
