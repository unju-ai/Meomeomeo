...
- **Combat extras:** shield absorb and a short WalkSpeed stun stub (server-authoritative). Flagship identity (guard, pull, takedown reset, breakable channel, mana refund, ticking zone) lives in `Shared.ChampionKits` and is applied only from `CombatService`.
