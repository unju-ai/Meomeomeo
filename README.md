...
- **Recall (F)** to fountain (amber progress swirl under feet), **match end screen** (punchy victory/defeat, CROWN MVP, your-row highlight, post/stall/core timeline + Levi take, Again · difficulty or Queue / lobby / Hub), clean return to lobby
...
- Combat VFX stubs: **pooled traveling bolts** for line Qs, ability beams/rings, AA claw + hit spark (debounced), tower bolts, structure death puffs, shield bubble, stun stars, recall progress swirl, **floating damage/heal numbers** (CombatFx float; merge + fog gated)
...
- **Closet** — hub or Cat Rift **Closet**. Starters Cream Cap + Yarn Puff. Buy **Gold Bell** for 25 closet yarn (start 40; play points, not Robux). Unlock Coral Beanie / Mint Aura from Yarn Run distance, Canal Crown from a legendary koi, Party Tiara + Moon Dust from a party win, Tape Shades / Braincell Orbs from arcade daily profit. Cat Rift adds **Lantern Cap** (last-hit an outer post or lantern stall), **Stall Spark** (win; Practice counts), and **Scratch Tally** (5 champion kills). Panel shows rarity + source chips, hover try-on, and louder `EQUIPPED ·` / `UNLOCKED ·` juice. A win pays 6 closet yarn, a loss 2. Drip shows in every mode; loadout and Rift counters save on `MeoCloset_v1` (Studio memory fallback). See PLAYTEST §3f.
...
- **Practice match** — from the Cat Rift stall: solo is you on Blue plus 2 ally bots vs **3 AI cats** on Red. Two humans keep 1 ally bot; three Blue humans keep none (Blue stays at 3). Toggle **Easy / Normal / Hard**. Attack bots, then each lane's scratching post, its lantern stall, then the nexus.
...
- **Q W E R** — aim with the mouse; the server validates range, mana, cooldown, and deals damage to enemy cats, **minions**, wards, and (if ungated) structures. The Cat Rift ability bar shows an ink cooldown sweep, a mint ready flash, and a coral tint when mana is too low (seconds and timings unchanged). Above the slots: coral HP and cobalt mana fills with **current / max**, plus an amber XP bar with a level label; low HP (<30%) and mana-gate conditions warn-pulse those tracks.
...
- **Tab** (hold) — scoreboard: Blue/Red sections, YOU highlight, dead dimmed, KDA / CS / gold / level / short items, team totals. Same server purse as the chip, including bots. Non-modal. A teammate idle for **90s** gets a small **AFK** chip.
- **AFK** — alive in a Cat Rift fight with no move, auto-attack, cast, ping, shop, or recall for **45s**: local toast **Still there?**. At **90s** teammates see the Tab chip and a quiet `{name} is away.` line. Any of those inputs clears it. Dead time and a disconnect freeze the stamp. Bots ignored. No kick, no surrender, no bot. Other stalls stay quiet. A rejoin sets `connected` true again so remake's missing-teammate check clears.
- **F7** or the **Remake** chip (just left of **Fold**) — Cat Rift remake while the clock is **under 3:00**. Same living-human majority. Practice can start anytime in that window; a real fight needs a teammate who never connected or already left. 30s window, then a **60s** cooldown. Pass ends with no winner (**REMAKE** / fight called early). Bots never vote. Other stalls stay quiet.
- **F8** or the **Fold** chip (beside **?**) — Cat Rift surrender after **8:00**. Majority of living human teammates (one human can pass alone). Bots never vote. 30s window, then a **90s** cooldown. Practice included. Other stalls stay quiet. A teammate who left and rejoins is connected again, so they are back in the living count when they are alive.
...
- **Minimap** (bottom-right) — lanes, towers, nexuses, allies, **fog overlay**, visible enemies/minions, and visible jungle camps (amber). Unseen enemies stay off the map. **You** are a cream pip with an ink ring; allies keep team fill + cream stroke; enemies keep team fill + coral stroke. Scratching posts are squares; open lantern stalls are amber circles (gated stalls are dim wood); the yarn core is pink. **Canal Levi** shows a bright square pit while UP in fog, a muted square while returning, a faint gray after taken, and a larger body dot only while the beast is in vision. Click it to ping teammates (short local Attention flash).
...
- **Kill feed:** top-of-screen night-market chips for kills, posts, lantern stalls, yarn core, level-ups, Kitty Caster announces, and team pings (`KillFeed` + `Shared.KillFeedPaint`). Your kills/deaths get a light cream/coral ring. Announce chips stay quieter and expire sooner so they do not fight her banner. Meme tape sits just under the stack.
...
- **Pawmart:** at your fountain (or talk to the clerk), press **B** and spend match gold on Longclaw / Yarnplate / Mana Treat / Pounce Boots / **Whisker Lens** / **Control Yarn** / **Yarn Cleave** / **Stall Fang** / **Paper Charm**. Server checks gold and location. The panel groups those nine under **Stats / Vision / Actives** with key chips on **5** / **6** / **7**, purse-matched gold, and buyable / owned / away tint (`Shared.ShopPaint` — display only). **Sell** on that same panel pawns one owned copy for 70% of the catalog price, rounded down, back onto the purse (`Shared.ShopSell`). Control Yarn is one charge per bag copy, so a leftover charge refunds 70% of 75g, not half of a two-charge bundle. Off the fountain the clerk refuses. A cooldown does not block the pawn and is not transferred. Practice bots do not sell. Longclaw raises AA damage. The three night-market items are hooks in `ItemKits`, not more flat stats. Costs stay in the 75–300 band: start gold is 500, so Cleave (280) still leaves a Control Yarn (75), Fang (300) is the ceiling (Longclaw plus Fang waits on a wave), and Charm (220) sits with the 200g boots and treats. **B is shop only — recall is F, trinket is 4, lens is 5, pink is 6, cleave is 7.**
- **Recall:** press **F** (not B). Server starts a 7s channel (`Config.Combat.RecallSeconds`), roots you, then `PivotTo` your fountain. A **new** move order cancels immediately (`IssueMove`: WASD / stick after you release the direction you were holding, or a ground click that walks without auto-acquiring). AA, attack-move, a validated ability, wards, lens, Yarn Cleave, **S**, or **F** again also cancel. Champion/minion/tower/jungle damage still cancels. A shove past `RecallMoveCancel` (2.5 studs) still cancels while you are rooted. Camera, help, emotes, pings, and opening Pawmart do not. Fountain regen still ticks while you channel. While the channel is up, the client paints an amber/cream progress swirl (allies a faint ring, enemies fog-gated). Success is a brief mint flash and a fountain puff; cancel shatters the swirl and pops **RECALL CANCELED**. The channel bar still clears as soon as the server drops the channel. Duration and cancel rules are unchanged.
- **Fountain regen:** alive + inside fountain radius → `48` HP and `56` mana per second (server tick). Out in lane it's the slow combat regen. Client paints calm mint juice (foot ring, soft wash, occasional floats, AbilityBar glow) while regenerating on the pad — same rates, no second regen system.
...
- **Farm:** `Shared.FarmCredit` is the CS rule. A last hit on a lane kitten or a jungle camp is +1 CS and that gold. Kill gold (180), assist gold (60), and tower gold (120) do not add CS. Pawmart spends gold and leaves CS alone. A fountain pawn puts 70% back on the purse and still does not add CS. The purse chip above the ability bar and Tab both read `gold` / `cs` / `level` off the match snapshot. The chip is Cat Rift **InProgress** only.
- **Match end:** nexus HP → 0 stops minion/jungle/tower/vision ticks, clears combat (including recall/death timers), and shows Victory/Defeat + team KDA + MVP + post/stall/core counts and a match-lifetime structure timeline (posts → stalls → core, plus Canal Levi take from `riverEpic` buffs). The end card paints a mint/amber or coral banner, a CROWN MVP chip, and highlights your row. Practice shows **Again ·** the difficulty it will reuse; a live fight (including a live remake) shows **Queue**, which leaves the seat and joins the same queue as the lobby button (a party leader requeues the party). **Back to lobby** / **Hub** stay and, with that left chip, skip the 45s timer. **Ended counts as busy** so queue/practice cannot start underneath the screen. The timer still auto-returns so nobody soft-locks. Kitty Caster + kill-feed announce “Enemy nexus destroyed!” (per-team Victory/Defeat). Stats math and MVP pick are unchanged.
- **Minimap:** client paints the server fog mask (unexplored / explored-but-unseen / currently seen); click-to-ping is team-only (`PingReceived`) with a short local Attention flash. Unseen enemies never get a dot. Structure and Canal Levi markers use `Shared.MinimapPaint` (posts / stalls / core match smart-ping shapes; Levi UP / respawn / taken / body are distinct). Explored tint survives leaving vision and mid-match reconnect.
...
Screen juice (`src/client/Juice/ScreenJuice.luau`): coral damage flash, mint heal flash, cream/amber **LEVEL UP** pop (local Cat Rift level edge), soft mint/amber victory or coral defeat sting on the end screen, `CameraFollow.shake` on tower/nexus. Level-up also drives `CombatFx.localLevelUp` (silhouette ring + float) and an ability-bar rank pulse when `Progression.autoRanks` advances a slot. Fountain regen (existing CombatService pad ticks) drives a calm mint foot ring / soft wash / occasional `+HP`/`+MP` floats and AbilityBar stroke glow via `Shared.FountainRegenJuice` — pad ticks skip the loud heal flash.
...
| Fountain regen | Soft mint foot ring while topping up on the pad; occasional `+HP` / `+MP` floats; AbilityBar mint stroke glow; allies faint / enemies fog-gated |
