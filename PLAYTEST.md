...
1. **Hard** Practice. Get low (under about **40%** HP) in the open, next to a Hard ally bot, with an enemy cat within about **32** studs. Solo Practice is you versus three Red bots, so that ally is another Red bot: scratch one of them under **40%** and stand on them where a second Red bot can see you both (not in a brush that bot is outside). They should turn off a full-HP camp or the lane and walk onto the enemy. To be the low cat yourself, pick **Hard**, queue, and let bots pad the empty seats, then stand low next to a Blue Hard bot with a visible enemy nearby and watch that bot turn.
