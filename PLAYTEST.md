...
1. **Hard** practice. Red mid is the jungler. Blue mid, one of your two allies, runs the Blue route. Red top and Red bot stay in their lanes and last-hit, and the other Blue ally stays in Top. They should not cross the river to farm camps.
...
5. **Normal** practice, same look: mid only leaves for a camp when their wave is past the river, sticks to that camp until it dies, then goes back to mid. They are not on the Hard route. Blue mid does this too. **Easy** practice: all three Red bots and both Blue allies stay in lane. No bot should clear a camp.
...
6. **Practice:** leader **Practice with party** starts one in-place Practice. Party humans share **Blue**, and Practice still adds **2 Blue ally bots** plus **3 Red bots** (`PracticeAllies = 2`, `PracticeEnemies = 3`). Solo is 1 human + 2 allies vs 3 Red (3v3). A party of two is 4 Blue vs 3 Red. Allies use the same difficulty as Red. They lock a cat only after every human on Blue has locked, and they take Top and Mid so Bot stays open. **Practice again** on the end screen brings the same party. `MatchPlaceId` can stay `0`. Practice never reserves a server. Queue fill is still `QueueFillTo` (3 per side) and is not this count. `Config.Bots` is not asserted in a Luau smoke (server Config is outside the shared harness). Check the roster with compile plus the existing bot smokes below.
...
3. Practice: you are Blue with two ally bots, Red is three bots. If voice is actually up, the pill stays Ready and says bots are silent / no voice teammates. It should not look like a broken error. Ally bots are not teammates you can hear. There is nobody on the other side to hear you.
