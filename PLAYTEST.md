...
2. **Cat Rift Practice.** After draft, open **?**. Title **Cat Rift**. Scan **Movement · Combat · Vision · Shop · Social · Objectives**. Confirm rows for move / AA / QWER / B / F / 4 / 5 / 6 / 7 (via item rows) / G / T / Tab / **F7** / **F8** / V / Mute, fog + ghosts, death recap, purse/CS, Levi, posts → stalls → core, Invite, Kitty Caster, and Tips → Live (not a card). The Practice row reads solo + 2 Blue bots vs 3 Red, and a party caps Blue at 3. Kit shorthand still sits under QWER. **F7** says remake under 3:00. **F8** says surrender after 8:00, living humans, bots never vote.
...
4. Outside Practice, **F7** before anyone is missing toasts that a teammate has to have never connected (or already left). Once an allied human slot is missing, the connected living humans can start. Their Yes still needs a majority of whoever is still alive and connected — one remaining human passes alone.
