...
3. Press **7** next to a visible bot. A cream arc sits on the slash, wider and cream, not the thin team-colored auto-attack claw. Damage and the 12 second cooldown are unchanged. An ally nearby sees a fainter arc. Walk the bot into brush you are not in: the next **7** does not hit them, and you should not see an enemy arc from inside that brush.
