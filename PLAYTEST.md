...
Client juice only. Gold amounts, CS credit, and the purse chip stay in `FarmCredit`. A last hit is the credit with CS (`FarmCredit.lastHit`). Kill, assist, and tower gold are bounties and must not show this float. Smoke: `python3 tools/test-farm-credit.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Last-hit a lane kitten (the killing scratch). An amber **+18g** rises at the corpse with a tiny cream **+1 CS** under it, and a soft coin tick. The purse chip gold and CS both climb by that same last hit.
...
3. Kill a Red bot. The purse gains kill gold. No **+Ng** / **+CS** float — that gold is a bounty, not CS. An assist, or knocking a scratching post, also bumps gold with no CS float.
4. Optional: clear a jungle camp. **+Ng** matches that camp's gold and CS goes up by one. Canal Levi's take still shows the pit burst and the purse **+8%** line; the last-hit float is only the camp gold.
