...
3. With no vision on it, the minimap shows a **square pit** at that canal spot, not the body if someone has dragged it. Walk a ward or a cat into vision: the living body appears (a larger dot), the world model is visible, and the epic HP bar is readable. Step back out: the body and bar hide again. Fog rules are the same as any camp. The pit marker is still the only map mark. The clock is not a second pit.
...
2. Dump mana (spam casts / stand without Mana Treat). With a slot off cooldown but mana too low, that slot tints **coral** even though the seconds are gone. Regain mana: coral clears and the slot reads ready again.
...
4. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade do not show this bar. Floating damage and HP bars behave as before.
