...
Build verification on 2026-09-22: Luau compiled 117 sources; combat projectile + original-six kit hooks (Meow guard/pull, Nyan reset/channel, Whiskers refund/zone, Chonk Settled Loaf/Charge Knock, Scammy Open Wick/Hype Candle, Grandma Second Helping/Sweater Aura; other 8 expansion cats on the zero plan) + recall cancel-on-order + Control Yarn stacks + structure layout/gates (outer posts, inner lantern stalls, yarn-core gate) + vision/fog (mesh LoS: eye-height walls + thick cover, mesh raycast, 10-stud grid, match-lifetime explored OR, last-seen ghost freeze/fade) + jungle-bot route/commit/rotate + brand/lobby + cosmetics/arcade/koi/yarn/party (lobby seat takeover) smokes passed; Rojo 7.4.4 builds the place. This is build verification, not a Studio playtest.
...
The Luau smoke checks the hook table (guard math, pull, takedown reset slots, channel interrupt numbers, mana refund, zone pulses) and that the eight expansion cats stay on the zero plan. It does not move a character. Walk these three in Studio Play:
...
4. Lock **Bytekit** afterward. Expansion cats do not gain a guard, a reset, a breakable beam, a lingering zone, a knock, a wick, or a sweater. **Chonk Knight** is in the original-kit pass below.
5. Hub modes, fog, party queue, towers, and jungle bots behave as before.

### Studio live-pass (original kits)

The Luau smoke names **Settled Loaf**, **Charge Knock**, **Open Wick**, **Hype Candle**, **Second Helping**, and **Sweater Aura**, and checks that Bytekit and the other expansion cats stay on the zero plan. It does not move a character. Walk these three in Studio Play. Meow, Nyan, and Whiskers should still match the flagship pass above.
...
4. Lock **Bytekit** afterward. Those buttons stay ordinary: no Guard, no Wick, no cream aura, no knock.
5. Hub modes, fog, party queue, towers, and jungle bots behave as before.
...
- **Original kits:** Meow, Nyan, and Whiskers stay as shipped. Chonk's **E** is Settled Loaf (short damage cut) and **Q** is Charge Knock (bowl along the roll). Scammy's **Q** is Open Wick (stack on a cat, max 3) and **E** is Hype Candle (spends the stacks on cats only). Grandma's heals are Second Helping (×1.22) and **W** is Sweater Aura (ally damage cut). Expansion cats stay on the zero plan.
