...
Cat Rift early remake (same day, later slice): while the match clock is **under 3:00**, a human can press **F7** or the **Remake** chip just left of **Fold** to open a 30s ballot. Pass still needs the same living-human majority as surrender (1 alone, 2 need both, 3 need 2). Bots never vote. Dead or disconnected humans are out of the count. **Practice** may start anytime inside that window so a solo fight is testable. A real fight may start only when an allied human never connected or already left. Fail or timeout waits **60s** before that team can try again. A team cannot open remake while its surrender ballot is open, or surrender while its remake ballot is open. Pass ends the match as a **remake**: no winner, the end screen reads **REMAKE** / **Fight called early**, and Kitty Caster does not call victory or defeat. Surrender's 8:00 unlock, 30s window, and **90s** cooldown are unchanged. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Verified locally: Luau compiled 183 sources; `tools/test-remake.py`, `tools/test-surrender.py`, `tools/test-end-screen.py`, and `tools/test-announcer.py` passed; full smoke suite passed (35). Rojo 7.4.4 built the place. Studio live-pass under remake vote.

Cat Rift AFK warning (same day, later slice): a human who is alive in a fight and has no move, auto-attack, cast, ping, shop, or recall for **45s** gets a local toast **Still there?**. At **90s** teammates see a small **AFK** chip on that row of the hold-**Tab** scoreboard, and a quiet kill-feed line `{name} is away.`. Any of those inputs clears the chip and resets the stamp. Dead time and a disconnect freeze the stamp, so a recent action is not eaten by a death timer. Fountain regen and a recall channel are not walks; the recall press itself is input. A rejoin sets that seat's `connected` back to true on **PlayerAdded** and again on character restore, so remake's missing-teammate check clears. Bots are ignored. Nothing surrenders, kicks, or spawns a bot. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Verified locally: Luau compiled 185 sources; `tools/test-afk.py`, `tools/test-remake.py`, and `tools/test-surrender.py` passed; full smoke suite passed (36). Rojo 7.4.4 built the place. Studio live-pass under AFK warning.
...
Recall channel juice (same day, later slice): the 7s channel, cancel-on-order, and fountain teleport stay in `CombatService` / `RecallLogic`. Client paints them. While the local cat is recalling, an amber/cream foot swirl fills from `recallStartedAt` → `recallEndsAt`, with a soft silhouette wash and a quiet tick under the existing recall loop. Success (duration 0 at the fountain spawn) is a brief mint flash plus a pad puff. Cancel (duration 0 at the zero-origin sentinel — move, cast, or damage) shatters the swirl coral and pops a short **RECALL CANCELED** banner. Allies may see a faint ring; enemy rings stay fog-gated. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Fountain regen, walls, nameplates, kits, and Pawmart untouched. Verified locally: Luau compiled 152 sources; `tools/test-recall.py` passed (progress / success / cancel / fog); `tools/test-combat.py` passed; Rojo 7.4.4 built the place. Studio live-pass under recall channel juice.

Control Yarn / ward juice (same day, later slice): place range, duration, stacks, health, and vision stay in `WardService`. The part now also carries owner, `MeoPlacedAt`, and `MeoExpiresAt` so the client can paint them. A fresh part (placed within about 1.25s) gets a soft cream/amber puff; allies get a cream/amber yarn bob and lantern above the existing ball or pillar. Enemies get that silhouette only when the ward id is already in the vision list (pink control stays visible to everyone; stealthed trinkets stay hidden). The last ~12s pulse and fade. When a ward you were showing leaves the folder, a short pop plays; a hidden expire does not. The planter gets a short **Control Yarn planted** / **Ward planted** float. Failed plants still toast the existing refuse line. Minimap dots stay on the same visible-id set and only grow fainter near expire. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Recall, fountain regen, walls, kits, and Pawmart prices untouched. Verified locally: Luau compiled 154 sources; `tools/test-ward-juice.py` passed (place window, ally vs fog, fade, pop); `tools/test-items.py`, `tools/test-vision.py`, and `tools/test-minimap.py` still passed. Rojo 7.4.4 built the place. Studio live-pass under Control Yarn juice.

Death → respawn juice (same day, later slice): the respawn timer, death recap lines, and gold loss stay in `CombatService` / `DamageLog`. While the local cat is down, a center-top cream/amber chip counts the existing `respawnAt` (`12s`, or `1:05` once a minute is left) and a soft ink wash sits behind the HUD. The last 3 seconds pulse. On the alive edge, a brief mint pad puff and silhouette wash play at the fountain — no heal flash and no Heal ping, so it does not stack on fountain regen. Allies may get a faint puff plus a quiet **back** at their pad. Enemies get that puff only when their fountain is already in vision. The recap card still hides on its own timer; **Recap** still reopens it. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Ward juice, recall, fountain rates, kits, and Pawmart untouched. Verified locally: Luau compiled 156 sources; `tools/test-combat.py` passed (RespawnJuice clock / pulse / fog / arrival); `tools/test-death-recap.py` still passed. Rojo 7.4.4 built the place. Studio live-pass under death respawn juice.

Pawmart active juice (same day, later slice): Whisker Lens, Yarn Cleave, Stall Fang, and Paper Charm keep their costs, cooldowns, damage, and buy rules. Cat Rift paints the uses that already exist. **5** is a short cream glance plus an eye flash at you — it does not draw the 32-stud reveal and it does not mark a ward you cannot see. **7** is a cream arc on the existing 12-stud slash, wider than an auto-attack claw. Stall Fang's larger execute number stays; a low-HP champion hit also gets a short coral bite and, for you, a brief silhouette flash. Paper Charm puffs pink when an enemy pink opens the 2.4s window, then a soft pop when that window ends — not a second hit. You read your own cue clearest. Allies get a fainter version. Enemies get it only while the body is already in fog. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 157 sources; `tools/test-combat.py` passed (ActiveJuice mode / fog / charm window edge); `tools/test-items.py` still passed. Rojo 7.4.4 built the place. Studio live-pass under Pawmart active juice.

Canal Levi take juice (same day, later slice): spawn at **3:00**, the one ~90s return, 1100 HP, and the non-stacking **+8%** stay in `RiverEpic`. A last hit still publishes the phase leave and the purse flags on one snapshot. The client paints a brief cream (Blue) or coral (Red) burst at the canal pit. Your side's take shows through fog; an enemy take stays quiet unless the pit is lit now or was on the previous mask. While a team holds the buff, living cats on that side get a calm team-tinted silhouette rim. Yours is a little clearer. Dead cats and the side that does not hold it stay bare. The rim clears when the flag is gone or the cat is down. Kitty Caster already calls the take, so there is no second banner. No +8% float on a timer — the purse line is still **Canal Levi · +8% damage**. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Structure falls, actives, recall, respawn, and wards are unchanged. Verified locally: Luau compiled 159 sources; `tools/test-combat.py` passed (take tint, fog, rim, no float); `tools/test-jungle-bot.py` passed (priority unchanged). Rojo 7.4.4 built the place. Studio live-pass: Hard Practice, wait for **Levi UP**, take it, confirm the pit burst, the holder rim, and the purse line.

Last-hit gold float (same day, later slice): lane kitten and jungle camp last hits still go through `FarmCredit.lastHit` (one CS plus that gold). Kill, assist, and tower / nexus gold stay `FarmCredit.bounty` (gold, no CS). After a successful last-hit credit, the server emits `CombatFx` `kind = "lastHit"` with that same gold and the corpse position. The client paints only for the local cat: amber **+Ng** with a tiny cream **+N CS**, merged on the CombatFloat window so a cleave does not carpet, plus a soft coin tick on the first pop of a burst. Allies and enemies do not get the float. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Purse math is unchanged. Levi's pit burst and +8% rim are unchanged. Verified locally: Luau compiled 160 sources; `tools/test-farm-credit.py` passed (bounties do not paint); `tools/test-combat.py` passed (label, merge, local, mode). Rojo 7.4.4 built the place. Studio live-pass under last-hit gold.

Brush enter juice (same day, later slice): the pockets that hide you stay `VisionLogic.brushAt` (the same AABBs `MapBuilder` already paints). Vision radius, brush stealth, and the fog mask are unchanged. While Cat Rift is in progress, the local cat gets a calm night-market wash on the edge: a few mint leaves and a quiet foot hush plus a slight darken on enter, a very light green foot tint while you stay, and a cream fade when you leave. A visible ally may get a fainter hush. Enemies in brush are not marked. Last-seen ghosts, last-hit floats, Levi's rim, and structure falls are unchanged. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 161 sources; `tools/test-vision.py` passed (enter / stay / leave, ally only if already visible, enemy never, hub quiet, pocket still hidden). Rojo 7.4.4 built the place. Studio live-pass under brush juice.

Bounty gold float (same day, later slice): kill gold (180), assist gold (60), and tower / nexus gold (120) stay `FarmCredit.bounty` (gold, no CS). Amounts are unchanged. After a successful bounty credit, the server emits `CombatFx` `kind = "bounty"` with that same gold — at the victim for a kill or assist, at the structure for a take. The client paints only for the local cat: peach **+Ng**, no CS line, merged on the CombatFloat window and soft-capped so a teamfight does not carpet, plus a soft coin tick on the first pop of a burst. Last-hit kitten and camp gold stay the amber **+Ng** / **+CS** float on `kind = "lastHit"` and are never sent on the bounty path. Allies and enemies do not get the float. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Brush wash, Levi's rim, and structure-fall world FX are unchanged. Verified locally: Luau compiled 162 sources; `tools/test-farm-credit.py` passed (bounty paints, last hits do not); `tools/test-combat.py` passed (peach label, merge, local, mode). Rojo 7.4.4 built the place. Studio live-pass under bounty gold.

Structure aggro cue (same day, later slice): posts, lantern stalls, and the yarn core keep their range, damage, shot interval, and `TowerService.pickTarget` order (recent attacker in range, else nearest champ, else kitten). Each living structure writes that champion id onto `MeoLockUserId` even while the bolt is cooling, and clears it for a kitten or an empty range. The existing `towerShot` event now also carries that champion id. The client paints a soft coral foot ring on the structure and a thin coral ring under you while you are the pick. A stall adds a warm lantern glare; the bolt stays lantern gold. When that bolt hits you, a short coral flash rises — the damage number is still the old float, not a second hit. A teammate's lock is a fainter ring, and only if that structure is in vision. Fogged allies, enemy dives, and kitten bolts stay quiet. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Bounty floats, brush, Levi, and structure-fall FX are unchanged. Verified locally: Luau compiled 163 sources; `tools/test-structure.py` passed (lock cue, fog, hub quiet, shot numbers unchanged); `tools/test-combat.py` passed. Rojo 7.4.4 built the place. Studio live-pass under structure aggro.

Jungle camp respawn juice (same day, later slice): pigeon / golem / crab still die into `JungleService`'s existing pending stamp (32s / 45s / 50s). Gold is unchanged. Canal Levi is not in that list — its line stays on the match clock. While a small camp is down, the vision frame includes that stamp only when the pit cell is lit now or already explored this match. The client paints a small cream/amber countdown at the pit and a faint square plus the same seconds on the minimap. An unexplored pit stays dark. When the stamp is reached, a brief soft puff plays and the countdown leaves; the body uses the normal camp presence rules. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Tower aggro, bounty floats, brush, and the Levi rim are unchanged. Verified locally: Luau compiled 165 sources; `tools/test-jungle-bot.py`, `tools/test-vision.py`, and `tools/test-minimap.py` passed (chip reads the stamp, explored vs unexplored, Levi excluded, dim pit mark). Rojo 7.4.4 built the place. Studio live-pass under camp respawn.

Lane kitten wave telegraph (same day, later slice): lane waves still leave on `MinionService`'s existing waits — 6s to the first kitten, 0.35s between kittens, then 22s. Counts, gold, and pathing are unchanged. When that coroutine begins, it stamps the server time and those same numbers on the minion folder. The client paints from that stamp. About 2.5s before a lane's first kitten, a soft cream lantern glow sits on the Blue and Red mouths of that lane, and the minimap blinks a tiny cream dot there. A mouth you have not seen stays dark. When the kitten steps out, a brief cream puff plays and the glow leaves. Later kittens in the same lane do not repeat it. Jungle camp chips, tower aggro, and bounty floats are unchanged. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 167 sources; `tools/test-wave-juice.py` passed (lead, stagger, fog, hub quiet); `tools/test-minimap.py`, `tools/test-combat.py`, and `tools/test-structure.py` passed. Rojo 7.4.4 built the place. Studio live-pass under lane wave telegraph.

Cat Rift match start cue (same day, later slice): champion select, the teleport, and `beginFight` are unchanged. The server still sets `phase = InProgress` and `startedAt` to the server clock in the same step, clears `phaseEndsAt`, and does not freeze movement. The client reads that edge. Because the stamp is already "now", there is no free 3s pre-roll, so Practice shows one cream/amber **Fight!** — not a countdown that would invent a freeze. A stamp that is already about 3s ahead would count **3 · 2 · 1** and then **Fight!**; the live match does not do that. The begin cue is a short center word, a soft amber wash, a cream fountain-pad glow on your pad, and a brief silhouette wash. A quiet ping sits under Kitty Caster's existing **WE ARE LIVE** line. Her script is unchanged. The overlay does not take clicks. A rejoin after the fresh window stays quiet. Spectators stay quiet. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Wave telegraphs, camp chips, and tower aggro are unchanged. Verified locally: Luau compiled 169 sources; `tools/test-match-start.py` passed (abrupt Fight!, full 3s count, late rejoin, hub quiet, no input lock). Rojo 7.4.4 built the place. Studio live-pass under match start.

XP float (same day, later slice): nearby kitten XP (10), nearby camp XP (12), last-hit kitten XP (18), camp last-hit XP, kill XP (80), and assist XP (30) still go through `MatchService.grantXp`. Curves, share radius, and level thresholds stay in `Progression`. After a grant above zero, the server emits `CombatFx` `kind = "xp"` with that same delta and the corpse (or the cat, if the body is already gone). The client paints only for the local cat: a soft cream/lavender **+N XP**, above the amber last-hit gold and the peach bounty gold, merged on the CombatFloat window and soft-capped so a wave does not carpet. Zero grants stay quiet. There is no coin tick. The grant that tips a level still paints **+N XP**; the cream/amber **LEVEL UP** banner stays the level-edge celebration and is not repeated by this tick. Allies and enemies do not get the float. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Match start, wave telegraphs, camp timers, and tower aggro are unchanged. Verified locally: Luau compiled 170 sources; `tools/test-farm-credit.py` passed (xp tick skips zero and gold); `tools/test-combat.py` passed (lavender label, merge, local, mode, curve untouched). Rojo 7.4.4 built the place. Studio live-pass under XP float.

Auto-attack scratch juice (same day, later slice): basic attacks still go through `CombatService.tickAutoAttack`. Damage stays **24** plus items, range **26**, interval **1.05s**, windup **0.16s**. The windup used to set a timer and return with no emit. It now sends the existing `CombatFx` `kind = "aa"` with `duration` set to that windup. The landing emit is the same `aa` with no duration, and the champion id when the target is a cat. `ScratchJuice` only decides the beat and who may see it. You get a short cream claw toward the target, then a narrow cream scratch arc. A cat who scratches you gets a tiny cream tick — the damage float is still the only number. Allies see a fainter claw and arc. Enemies see them only while that attacker is already in vision. Yarn Cleave on **7** keeps the wider arc. Skillshot bolts stay bolts. Hub / Yarn Run / Koi / Party / Arcade stay quiet. XP floats, tower aggro, wave lanterns, and the Fight! cue are unchanged. Verified locally: Luau compiled 171 sources; `tools/test-combat.py` passed (ScratchJuice windup vs hit, fog, narrower than Cleave, no number). Rojo 7.4.4 built the place. Studio live-pass under auto-attack scratch.

Ability refuse cue (same day, later slice): Q/W/E/R and **4** / **5** / **6** / **7** still ask the server. Mana costs, cooldowns, and range checks are unchanged. When that call comes back refused, the client reads the existing error. **Ability on cooldown**, **Trinket on cooldown**, **Lens on cooldown**, and **Yarn Cleave is cooling down** paint **On cooldown**. **Not enough mana** paints **Need mana**. **Out of range** and **Ward out of range** paint **Out of range**. The refused slot gets a short coral wash. A soft coral tick crosses the screen, fainter than a hit. Mashing inside about half a second paints once. A cast that lands does not use this path. Down, unlearned, and "buy at Pawmart" keep their old line. There is no silence rule and no ability "no target" rule, so those are not painted. Auto-attack misses stay on the attack toast. The cooldown sweep, aim rings, scratch arc, and gold / XP floats are unchanged. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 172 sources; `tools/test-combat.py` passed (AbilityRefuseJuice reasons, hub quiet, debounce); `tools/test-resource-bars.py` passed. Rojo 7.4.4 built the place. Studio live-pass under ability refuse.

Ability cast confirmation (same day, later slice): Q/W/E/R and **4** / **5** / **6** / **7** still ask the server. Mana costs, cooldowns, and range checks are unchanged. When that call comes back `ok = true`, the slot that cast gets a short cream wash and an amber rim. A very soft cream tick crosses the screen, quieter than the coral refuse tick. There is no banner. Pressing the same slot again to break your own channel does not paint — that press is a cancel, not a new cast. Spam on cooldown still uses the coral refuse. Kit bolts, zones, and the scratch arc stay as they are. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 173 sources; `tools/test-combat.py` passed (AbilitySuccessJuice ok flag, hub quiet, quieter than refuse); `tools/test-resource-bars.py` passed. Rojo 7.4.4 built the place. Studio live-pass under ability cast.

Hard bot siege (same day, later slice): `BotSiegeLogic` picks a vulnerable enemy post or open stall the wave is already under. A healthy post needs two allied kittens and a winning wave. One kitten is enough when that post is already focused or at half HP. Zero kittens never, and the yarn core never. Hard lane bots and Hard mid walk up and scratch it through the existing objective attack. They stay through the tower bolt that their own scratch paints, and they leave when the wave is gone or their HP hits the retreat line. Peel, Canal Levi (unless the existing critical nearby peel), a hurt camp, a cat in face range, and low-HP flee still win. Easy and Normal are unchanged. Control Yarn, cleave, and lens are unchanged. Tower HP and gold are unchanged. Verified locally: Luau compiled 175 sources; `tools/test-jungle-bot.py` passed (wave join, empty refuse, focused/low with one kitten, peel / Levi / hurt camp / face-range above siege, Easy and Normal ignore); `tools/test-structure.py` still passed. Rojo 7.4.4 built the place. Studio live-pass under Hard siege.

Hard bot finish (same day, later slice): `BotFinishLogic` scores a visible wounded enemy champion. At or under **40%** HP inside about **64** studs, or at or under **28%** inside about **112**, Hard leaves a full-HP camp or a healthy last-hit and walks in to scratch that cat. An empty post is still not a dive. Low-HP flee, peel, Canal Levi, a camp they already hurt, a cat already in face range, and a wave that is already taking a post still win. Easy and Normal hunt the same as before. Ability chance, Stall Fang, and tower damage are unchanged. Verified locally: Luau compiled 176 sources; `tools/test-jungle-bot.py` passed (40% within 64, 28% within 112, empty-tower refuse, finish beats a full-HP camp and a lane shove, peel / Levi / hurt camp / siege / face-range above finish, Easy and Normal ignore); `tools/test-structure.py` still passed. Rojo 7.4.4 built the place. Studio live-pass under Hard finish.

Hard bot core (same day, later slice): `BotCoreLogic` picks the enemy yarn core only after `StructureLogic` has already opened it (every post and lantern stall on that side down). A winning wave is two allied kittens and more than theirs. Half HP, or two allies already in scratch range, is enough without that wave. An empty full-HP core is not a solo walk, including from the fountain. Outer and stall siege still never goal the core. The walk uses the existing objective attack and the same dive check, so an empty full core still will not step into nexus range. Peel, Canal Levi while it is up, a hurt camp, low-HP flee, and a wounded cat already in face range still win. The open core beats a full-HP camp, an idle rotate, and a wounded cat outside face range. Easy and Normal are unchanged. Core HP and gold are unchanged. Verified locally: Luau compiled 177 sources; `tools/test-jungle-bot.py` passed (winning wave, half HP, two allies, empty full refuse, gated refuse, peel / Levi / hurt camp / face-range above the core, core above a full-HP camp and a far finish, Easy and Normal ignore); `tools/test-structure.py` passed (stalls down open the core, a gated core stays closed). Rojo 7.4.4 built the place. Studio live-pass under Hard core.

Hard fountain refuse (same day, later slice): `BotFountainLogic` uses the existing fountain regen disk (`FountainRegenJuice`, radius 32 at x = ±176, the same pad `ShopService.nearFountain` and fountain regen already use). A Hard finish whose cat or next step sits on the **enemy** pad is dropped, and the bot falls back to the lane, a camp, or a siege. Easy and Normal share that disk on a chase step. Standing on the enemy pad without an open-core siege, they step back to the lane side of the rim. Their own fountain is still recall, shop, and regen. An open yarn core with a wave still sieges; the core is not that pad. Fountain damage, regen rates, and human movement are unchanged. Verified locally: Luau compiled 178 sources; `tools/test-jungle-bot.py` passed (finish on the pad refused, finish outside still happens, ally pad still finishes, open core still sieges inside the disk). Rojo 7.4.4 built the place. Studio live-pass under Hard fountain.

Structure take juice (same day, later slice): outer posts, lantern stalls, and the yarn core keep their HP, gold, and `StructureLogic` gates. When one dies, the existing `structureDeath` event paints the fall: wood planks and a cream collapse for a post, a warmer lantern burst for a stall, pink yarn strands for the core. If that death opens the next structure, a short mint and wood cue plays at the gate. Enemy falls stay quiet unless that site is in vision (or was on the previous mask). Ally-side falls still show through fog, same as ally structure bars. A soft camera thump is local and only when you are already next to a take you can see; the kill-feed shake is unchanged. Kitty Caster lines are unchanged. Hub / Yarn Run / Koi / Party / Arcade stay quiet. Verified locally: Luau compiled 158 sources; `tools/test-structure.py` passed (looks, fog, opened gates); `tools/test-combat.py` passed. Rojo 7.4.4 built the place. Studio live-pass under structure take juice.
...
3. **Wards**. **4** is the free stealthed trinket, one live, about 70 seconds, lasting about 60. Control Yarn is 75 gold with **B**, two charges, then **6**: a magenta pink everyone can see, about 90 seconds, slowing enemies inside about 22 studs and revealing nearby enemy trinkets. Whisker Lens is 180 gold, then **5**: 80 damage inside about 32 studs (a trinket at 60 pops, a full pink at 90 does not), then about 75 seconds. Your pink slows enemies, not you. Paper Charm shrugs an enemy pink for about 2.4 seconds, then waits about 20. Hard bots drop one free pink, then buy Control Yarn on a later visit and plant it toward the river.
...
1. Hub → **Cat Rift → Play** → bot difficulty **Easy**, **Normal**, or **Hard** → **Practice match**. On the hub grid first: hover the Cat Rift stall (glow + lift), confirm the one-line blurb, then **Play**. **← Hub** returns to the grid. After lock-in, a cream/coral **tip card** (top-left) walks move, scratch, abilities, the kit cards, **purse + Pawmart** (Yarn Cleave on **7**, Stall Fang, Paper Charm), recall, wards, fog, **outer post → lantern stall → yarn core**, death recap, Kitty Caster + named **G** pings, Invite / party queue, then Closet (Lantern Cap / Stall Spark) and **Mute me**. **Next** or **Skip all**. Combat still works — the card is not a modal. Lobby **Show tips** replays the current deck anytime. Skip/finish persists (`MeoTutorial_v1` boolean, memory fallback in Studio). Players who already skipped or finished stay dismissed until they press **Show tips**. Hub **Live** is on **H / ?** (Tips row), not a card. Yarn Run, Koi Pond, Yarn Party, and Meme Arcade do not show the card.
   ...
   - **Hard** — faster, 1.22× damage, tighter CS, a short role build bought at the opening fountain (bruiser/tank: Yarnplate + Paper Charm, assassin: Longclaw + Pounce Boots, mage: Mana Treat + Longclaw). Stall Fang, Yarn Cleave, Control Yarn, and Whisker Lens wait until a later visit can pay for the next one. Control Yarn is two charges after Fang or Cleave and before the lens. One early **magenta control (pink)** ward once they have left the fountain (still free, before they own Control Yarn). After they own a charge, they plant it toward a river brush, the canal, or a crab approach when they are close and not under an enemy post — one plant per charge, and not while that pink is still up. Presses **7** when two visible enemy cats are inside the cleave circle. Uses Whisker Lens when an enemy pink or trinket is inside the lens radius, and only after they own it. Kill-dives. Mid runs pigeon pack → yarn golem → near river crab → far river crab, then rotates to the pushed lane. Top and bot stay in lane. If a visible ally is at or under about **40%** HP and a visible enemy is within about **32** studs of that ally, they leave a full-HP camp, a soft hunt, or the lane and walk onto that enemy. A camp they have already hurt still finishes. Low-HP flee and an unsafe tower still win. Canal Levi still wins unless that ally is at or under about **22%** and the fight is within about **48** studs. Easy and Normal do not peel. When allied kittens are already under a vulnerable enemy post or an open stall — two kittens on a healthy post, or one if that post is already hurt — a healthy Hard bot walks up and hits it instead of a full-HP camp or last-hit farming. They do not step under an empty post, and they do not all-in the yarn core from the fountain. Peel, Canal Levi, a hurt camp, and low-HP flee still come first. Easy and Normal do not take that walk.
2. You are Blue: you, **2 Blue ally bots**, and **3 Red (Bot)** cats (3v3). The Practice difficulty toggle is the same for allies and Red. Ally bots do not lock a cat until you do, so Whiskers / Bytekit / Nyan Rocket stay open. After lock-in they take **Top** and **Mid** (Mid jungles on Normal and Hard). **Bot** is the open lane. Draft a cat (Professor Whiskers / Bytekit / Nyan Rocket are easy to read). Cards show a color swatch + ears, a short blurb, and a kit teaser (`passive · signature`). Role chips filter the 14-cat scroll. After lock-in, you and the bots should have **distinct silhouettes** (ears/tail/archetype flair) and nameplates (`Champion · role · look`, bots keep `(Bot)`, your plate can read **YOU ·**, plus a small team-tinted HP fill). The rift should read as a night-market: gold lane dots, indigo river, fountain lanterns, tower ears / yarn, **lantern stalls** (awning + paper lanterns) between the posts and each keep, jungle camp pedestals. **Fog of war** darkens ground outside ally vision. Your fountain, nearby living towers, and ally minions light bubbles. Red bots in river/jungle stay hidden until they walk into a bubble (plates hide with them). When one walks back out, a faint ghost lingers at the last spot, then fades.
3. Walk a lane. Unseen ground stays dark; the minimap matches (no enemy dots in fog). Walk back: cells you already lit stay a lighter **explored-but-unseen** tint (not full black). **LMB** a bot or minion — claw flash + hit spark. You cannot AA a target you cannot see. Hold **Q** if the kit is a line skillshot (thin mint range ring + width from the kit radius; coral when the cursor is past max range), **release** to fire a **traveling bolt** (hits on contact, server-authoritative; client VFX follows — bolts are not clipped by the ground fog overlay). Aim a bolt into a **keep wall** or a **market drum**: it should **stop / burst on the solid** and not continue through (see skillshot wall live-pass). Ground AoEs (Paw Slam etc.): **first press** shows the range ring and a clear aim circle at the clamped cursor, **click or press again** to confirm; **Esc / right-click** cancels and clears the ring. Instant heals still fire on press with **no lingering ghost ring**. Dashes stay hold-to-aim with a short streak + tip (not a full line bolt). Aim juice is local only — it does not reveal fogged enemies. The first-Practice cards include **Three flagship kits** (Meow guard + pull, Nyan reset + breakable beam, Whiskers mana refund + ticking zone), **Three more original kits** (Chonk Settled Loaf + Charge Knock, Scammy Open Wick + Hype Candle, Grandma Second Helping + Sweater Aura), **Three expansion kits** (Bytekit Packet Buffer + Overclock, Sir Scratchalot Honor Bleed + Shield Fortify, Shadowpounce Alley Mark + Smoke Vanish), **Three more expansion kits** (Chromeclaw Chrome Plate + Lunge Reload, Oracle Paws Ward Omen + Foresight Veil, Archmeow Spell Charge + Charged Meteor), and **Last two expansion kits** (Hexkit Curse Stacks + Hex Zone, Mindwhisker Psi Mark + Mind Nudge). Draft blurbs and **H / ?** repeat the short versions.
...
5. **F** recall (7s) — amber/cream foot swirl that fills while you channel, plus the channel bar. Stand still and it finishes with a brief mint flash and a fountain puff. A **new** WASD press, a **ground click**, attack, attack-move, a cast, or **7** (Yarn Cleave) cancels immediately: the swirl shatters coral, a short **RECALL CANCELED** banner pops, and the channel bar disappears. A direction you were already holding does not cancel until you release and press again. Damage still cancels. **B**, **T**, **G**, **H**, and **V** do not. A **purse chip** (above the ability bar) shows your gold, CS, and level. **Hold Tab** scoreboard (Blue/Red sections, **YOU** highlight, dead dimmed, bots tagged, short items) uses the same numbers — see the Tab scoreboard live-pass.
...
4. **Again · Easy** (or **Normal** / **Hard** — the word on the chip) restarts Practice at that difficulty with the same party. **Back to lobby** returns to the Cat Rift stall. **Hub** opens the night-market grid. Auto-return timer still works if you wait. A live fight, and a remake of a live fight, puts **Queue** in that left slot instead (see §4). A Practice remake still says **REMAKE** and keeps **Again ·** the difficulty.
...
3. **Again ·** the difficulty. The card stays hidden. **Back to lobby** (the Cat Rift stall) and press **Show tips**. The deck starts again at **Move & look**, including those new cards. **Skip all**, then **Show tips** once more. It replays.
...
4. **Kittens.** Last-hitting lane kittens does not call her. Dying to lane kittens (or a camp) stays quiet unless that death is first blood, or it is the last cat on a side that had 2 or more. Red's three bots going down is an **ACE** when the finisher is not already on a double / triple / penta — one cat chaining them hears the streak instead. Blue is you plus two ally bots, so the last Blue cat down can be an ace too. A side that started as one cat still is not. A scratching-post execution can still get a short line once the 8-second gap allows it.
...
3. Knock the post over. Kill feed says scratching post. The post falls with wood planks and a cream collapse, and a short mint/wood cue marks the stall that just opened (structure take juice pass). The stall billboard drops `(gated)` and brightens. It now shoots minions and you (lantern-gold bolt, same aggro: recent attacker, else nearest champ, else kitten). Damage the stall: its HP bar appears the same way. Ally posts keep their bars even if the ground fog is dark; an enemy bar does not show through fog.
...
### Studio live-pass (structure take juice)

HP, gold, and which gate opens are unchanged. `tools/test-structure.py` checks the look (post / stall / core), fog, and that `openedIds` matches `StructureLogic.isVulnerable` before and after the fall. Walk the rest in Studio Play:

1. Hub → **Cat Rift → Practice**. Push mid until you can see the Red outer scratching post. Knock it down.
2. The post should shatter: wood planks, a cream collapse at the foot, and a small lantern spark. It should not be a single plain puff. Kill feed still says scratching post. Kitty Caster still names that post. Your camera may thump a little if you are standing next to it.
3. That lane's lantern stall drops `(gated)` and brightens. A short mint ring and two wood posts play at the stall — calm, not another collapse. Top and bot stalls stay gated. The yarn core stays `(gated)`.
4. Recall home, or step so a far enemy post is in fog, and let lane kittens take a post you cannot see. No shatter in the dark. The kill feed can still mention it. If one of your own posts falls while you are across the map, you can still see that collapse.
5. Snuff the stall you opened. Warmer lantern balls and a wood awning, not the post's cream collapse. When the last post or stall on that side falls, the mint/wood cue is at the yarn core and the billboard reads `(OPEN)`.
6. Unplug the yarn core. Pink yarn strands unravel, then the end screen. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade do not play these cues.
...
3. Scratch a post or stall: structure bars still work. Champion nameplates now carry their own small HP fill (see champion nameplates live-pass) — they do not replace these unit bars. Pawmart, bots, and kits behave as before.

Smoke (not a Studio substitute): `python3 tools/test-structure.py /path/to/luau`.

### Studio live-pass (lane wave telegraph)

Wave timing stays in `MinionService`. `tools/test-wave-juice.py` checks the 6s first wait, the 0.35s kitten step, the 22s interval, and that an unexplored mouth stays dark. Walk the rest in Studio Play:
...
`beginFight` still stamps `InProgress` and `startedAt` together. Draft is still 25s (or instant when every cat is locked). The first wave is still 6s. `tools/test-match-start.py` checks that the live stamp paints **Fight!**, that a future 3s stamp would count, and that Hub / Yarn Run / Koi / Party / Arcade stay quiet. Walk the rest in Studio Play:

1. Hub → **Cat Rift → Practice**. Lock a cat (or wait out champion select).
2. As the draft closes and you land on your fountain, a cream **Fight!** pops near the top of the screen with a soft amber wash. Your fountain pad gets a short cream glow, and your cat gets a brief cream silhouette wash. You can move immediately — the word does not eat clicks or root you.
3. Kitty Caster still says the market is live. There is no second announcer script. There is no **3 · 2 · 1** on this path, because the server does not hold a pre-roll.
4. Lane mouths still telegraph the first kittens a few seconds later, on the same cadence. Camp chips and tower aggro behave as before.
5. Back out to the hub, or open Yarn Run, Koi Pond, Yarn Party, or Meme Arcade. None of them play **Fight!**.

Smoke (not a Studio substitute): `python3 tools/test-match-start.py /path/to/luau`.

### Studio live-pass (champion nameplates)

Visibility + label helpers are in `tools/test-structure.py` / `tools/test-vision.py` (`ChampionPlate.show` / `nameLine`). Walk the rest in Studio Play:

1. Practice lock-in. Your plate reads **YOU ·** champion name, a **role · look** tag, and a small team-tinted HP fill under the text. Red bots show `Champion (Bot)` + role · look + fill. No default Roblox humanoid bar.
2. Scratch a bot in vision: the enemy fill drops (cream → amber → coral). Ally / your fill tracks damage the same way. Walk a Red bot into fog / brush you are not in: the body and plate hide together (same as AA refuse). Ally plates can stay when the ground fog is dark.
3. Structure / kitten / camp bars still use `BillboardHp` and are unchanged. Ability bar HUD, kits, minimap, and Pawmart behave as before.

Smoke (not a Studio substitute): `python3 tools/test-structure.py /path/to/luau` and `python3 tools/test-vision.py /path/to/luau`.

### Studio live-pass (Pawmart shop panel)

UI only. Costs, stack caps, fountain gate, clerk tips, and ItemKits hooks stay as they were. The Luau smoke checks `ShopPaint` sections, key chips, gold / fountain copy, and buyable / short / owned / away tints. It does not move a character. Walk this in Practice.

1. Hub → **Cat Rift → Practice**. Lock in. Stand on your fountain and press **B**. The panel opens with **STATS · sticks**, **VISION · lens & pink**, and **ACTIVES · night market**. All nine items are on the list.
2. Gold reads like the purse chip (`500g` at start, amber). The fountain line says **at fountain**. Cards you can afford show an amber edge; spend down or open on a 300g Fang with less gold — unaffordable cards go coral and do not click.
3. Buy **Whisker Lens** — toast says **Bought Whisker Lens · Pawmart**, the card flips to mint **owned**, and the **5** chip stays on the row. Buy **Control Yarn** twice: the title shows **1/2** then **2/2** and stops. **Yarn Cleave** keeps a **7** chip; Fang and Charm stay under Actives without key chips.
...
1. **B** at your fountain. Pawmart lists the original six plus Yarn Cleave, Stall Fang, and Paper Charm under **Stats / Vision / Actives**. Buy a second Control Yarn, then a third: the card stops at 2/2. Buy Whisker Lens, then try again: it stays owned. Key **5** still sweeps wards. Key **6** still plants the magenta ball.
2. Buy **Yarn Cleave** (280g). The HUD reads **7 Cleave ready**. Stand next to a visible bot and a kitten and press **7**. A cream arc (wider than the auto-attack scratch), both lose a chunk (about 40 before armor), and the chip counts down. Press **7** again immediately: the **7** chip pulses coral and the line reads **On cooldown**. A whiff (nobody in the 12-stud circle) still starts the cooldown. After about 12 seconds it reads ready. Press **7** during a recall: the swirl shatters, **RECALL CANCELED** pops, and the slash still goes off. A bot inside brush you are not in does not take the slash.
3. Buy **Stall Fang** (300g). Auto-attack a full-health bot and remember the chunk. Get that bot to about 20% HP and hit again: the chunk is larger (about 36 more before armor) and a short coral bite flashes on that cat. A full-health kitten does not get that extra bite. Your cat's Q W E R still do what that cat's kit says.
4. **Hard** practice, with **Paper Charm** (220g). Let Red mid leave the fountain and drop the free pink. Walk into that magenta circle. Walk speed stays full, a pink puff marks you, and the HUD shows **Charm** for about 2.4 seconds. When that window ends, a soft pink pop — no extra damage. Step in again before 20 seconds: you slow down, and the HUD counts **Charm**, with no second puff. After that cooldown, the next step into the pink opens the window again. Your own pink slows enemies, not you, so it does not pop the charm.
5. Hub modes, fog, party queue, towers, and jungle bots behave as before.

### Studio live-pass (Pawmart active juice)

Display only. Prices, cooldowns, the 40 cleave damage, the 36 fang bonus, the 20% threshold, the 2.4s charm window, and the 20s charm cooldown stay as they were. The Luau smoke checks who may see a cue and when the charm window counts as closed. It does not move a character. Walk this in Practice after the Pawmart items pass.

1. **B** at your fountain. Buy **Whisker Lens** (180g), **Yarn Cleave** (280g), **Stall Fang** (300g), and **Paper Charm** (220g). The register, gold, and key chips stay the same: **5** lens, **7** cleave, Fang and Charm still have no key chip.
2. Press **5**. A short cream sweep and an eye flash play on you. The lens cooldown is still about 75 seconds. Drop an enemy trinket in fog you are not in and press **5** again once it is ready: the sweep stays on you. It does not outline that hidden ward. A ward you can already see is still revealed by the server the same way as before.
3. Press **7** next to a visible bot. A cream arc sits on the slash, wider than the auto-attack scratch. Damage and the 12 second cooldown are unchanged. An ally nearby sees a fainter arc. Walk the bot into brush you are not in: the next **7** does not hit them, and you should not see an enemy arc from inside that brush.
4. Scratch a bot down to about 20% HP with **Stall Fang** owned. The next champion hit shows the larger coral execute number and a short coral bite on that cat. Your own hit also flashes their silhouette. A full-health hit has the normal number and no bite. A kitten at low HP does not bite.
5. **Hard** practice. Let Red drop the free pink and step in with **Paper Charm**. A pink puff marks you when the window opens. About 2.4 seconds later a soft pop plays, and you did not take a charm hit. Stepping in again before 20 seconds slows you with no puff. Hub stalls stay quiet if you leave the rift.

Smoke (helpers only, not a Studio substitute): `python3 tools/test-combat.py /path/to/luau` and `python3 tools/test-items.py /path/to/luau`.
...
1. **Hard** practice, solo (two Blue allies). Red mid is the jungler. Blue mid, one of your two allies, runs the Blue route. Red top and Red bot stay in their lanes and last-hit, and the other Blue ally stays in Top. They should not cross the river to farm camps. A two-human party has one ally on Top only, so there is no Blue mid bot.
...
3. Stand next to a healthy camp they are clearing (inside about 32 studs). They should turn on you. Let them get a camp under about 40% HP and show yourself again: they should **finish the camp** before chasing. If you only appear far up the lane, in fog, they should keep clearing. A visible wounded cat can pull them off a full-HP camp from farther than 32 studs — that chase is the Hard finish pass. A hurt camp still comes first.
...
5. **Normal** practice, same look, solo: mid only leaves for a camp when their wave is past the river, sticks to that camp until it dies, then goes back to mid. They are not on the Hard route. Blue mid does this too. **Easy** practice, solo: all three Red bots and both Blue allies stay in lane. No bot should clear a camp. A party that leaves only one ally has that cat on Top, in lane.
6. The Hard jungler still drops one free magenta pink after leaving the fountain, before Control Yarn is in the bag. Once a later fountain visit buys **Control Yarn**, that same bot plants a charge toward a river brush, the canal, or a crab approach when it is in reach and the bot is not under an enemy post. A second charge waits until that pink is gone. They also leave that fountain already holding Pawmart items (Tab — see the bot Pawmart pass). Fog, last-seen ghosts, inner stalls, recall, hub, Closet, Yarn Run, Koi Pond, and Yarn Party are unchanged.
...
### Studio live-pass (ability refuse)

Client juice on a refused cast. Costs, cooldowns, and the server checks stay as they are. `AbilityRefuseJuice` only names the error string that already came back. Smoke: `python3 tools/test-combat.py /path/to/luau`.

1. Practice, lock a cat with a mana cost (Whiskers or Bytekit). Cast **Q** once so it goes on cooldown. Spam **Q**. The **Q** slot pulses coral, the top line reads **On cooldown**, and a soft coral tick crosses the screen — quieter than taking a hit. Extra presses inside about half a second do not stack another line. The ink cooldown sweep still shrinks on its own clock. When the cast is ready and you land it, there is no coral refuse.
2. Dump mana (spam ready casts, or stand off the fountain with no Mana Treat) until **Q** is off cooldown and the slot is already coral from the mana gate. Press **Q**. The same slot pulses again and the line reads **Need mana**. Regain mana and cast: the bolt or slam goes out, the coral line stays off, and the cream slot flash is the ability cast pass.
3. A ground cast (Meow **Q**) aimed past its range and confirmed: the line reads **Out of range** and that slot pulses. Hold-to-aim lines still clamp and fire — they do not grow a new range error. **4** past ward range reads **Out of range** on the Ward chip. **5** or **7** pressed again while cooling reads **On cooldown** on that chip. A successful **7** is still the cream cleave arc, not this coral path.
4. Press an unlearned slot: the old "not learned" line stays, with no coral refuse. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Scratch arcs, gold / XP floats, and aim rings look the same. A cast that lands uses the cream confirmation in the next pass, not this coral one.

### Studio live-pass (ability cast)

Client juice when a cast comes back `ok = true`. Costs, cooldowns, and the server checks stay as they are. `AbilitySuccessJuice` only reads that flag. The coral refuse path is unchanged. Smoke: `python3 tools/test-combat.py /path/to/luau`.

1. Practice, lock any cat. Cast **Q** while it is ready and in range. The **Q** slot flashes a short cream wash with an amber rim. A very soft cream tick crosses the screen — quieter than the coral refuse tick. There is no banner. The bolt, slam, or scratch still plays as before. The ink cooldown sweep still starts on its own clock.
2. Spam **Q** while that cooldown is up. The slot pulses coral and the line reads **On cooldown**. The cream confirmation does not replace that refuse.
3. Land **W**, **E**, or a learned **R** the same way: cream on that slot only. **4** plants a ward and the Ward chip flashes cream (the yarn puff is unchanged). Buy Lens and Cleave: a successful **5** and **7** flash those chips; the glance and the cleave arc stay. **6** after buying Control Yarn flashes the pink chip when the plant lands.
4. Press the same slot again to break your own channel: no cream flash (that press is a cancel, not a new cast). Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet.
...
The Luau smoke checks who buys what out of 500 starting gold, when cleave or lens is allowed, and where a bought Control Yarn wants to land. It does not move a character. Hold **Tab** in Practice — the Items column is the bot's bag. Champion kits, fog, party, and the jungle route stay as they were.

1. **Hard** practice. Before the bots walk off the fountain, Tab should already list items. Blue allies use that same Hard plan. A bruiser (Chonk Knight, Sir Scratchalot, Chromeclaw) shows **Yarnplate** and **Paper Charm**. An assassin (Nyan Rocket, Shadowpounce) shows **Longclaw** and **Pounce Boots**. A mage (Bytekit) shows **Mana Treat** and **Longclaw**. Gold is lower than 500. They do not own the whole shop, and nobody lists **Control Yarn** yet (75g does not fit in the leftover purse, and it is not allowed to jump ahead of Fang or Cleave).
2. Let them farm, then meet one back at the fountain. The next purchase is the next item they can afford, in order: assassins pick up **Stall Fang** (300g) only after Longclaw and boots, then **Yarn Cleave** (280g), then **Control Yarn** (75g, twice), then **Whisker Lens** (180g). Bruisers and mages pick up Cleave once they have 280g, then Control Yarn, then the lens. A second visit with gold still short does not buy a later cheaper item ahead of that — 74g after Cleave does not buy the lens, and 255g does not buy Control Yarn while Stall Fang is still next. When the list is owned, further gold does not add more rows. Control Yarn stops at 2.
...
4. After Tab shows **Control Yarn**, follow that Hard bot toward the river (brush by a crab, the canal, or the crab itself). When they are close and not standing under your post, they plant one magenta pink and the charge drops by one. They do not immediately drop the second charge on top of that pink. If you kill the pink, the next charge can go down on a later pass, still not under your post. Before the yarn is on the row, the only magenta pink is Red mid's free one.
5. After Tab shows **Whisker Lens**, drop a trinket (**4**) or a pink (**6**) inside about 32 studs of that bot. They sweep it. Before the lens is on the row, they do not. Red mid still plants one free magenta pink before Control Yarn is owned.
6. **Easy:** the Items column starts as **—**. If one limps home, Tab shows at most **Mana Treat**. **Normal:** they leave naked; a fountain return adds two stat items and never Cleave, Fang, Charm, or Control Yarn.
7. Hub modes, fog, party queue, towers, and the jungle route behave as before.

Smoke (not a Studio substitute): `python3 tools/test-bot-shop.py /path/to/luau`.

### Studio live-pass (Hard peel)

Hard bots only. Easy and Normal keep farming. No new damage, items, or buttons. Control Yarn, Cleave, and Lens stay as they are. The jungle smoke checks who they turn for. It does not move a character.

1. **Hard** Practice. Solo Practice is you plus two Blue bots versus three Red bots, and those Blue bots are on Hard too. Get low (under about **40%** HP) in the open, next to a Blue ally (Top, or Mid once they are off a camp), with an enemy cat within about **32** studs (not in a brush that bot is outside). They should turn off a full-HP camp or the lane and walk onto the enemy. A Red-versus-Red peel still works if you scratch one Red bot under **40%** and stand where a second Red bot can see you both.
2. A camp that bot has already hurt still finishes. If the bot is itself running home, or the fight is an unsafe tower dive, they do not turn. Fog still hides an enemy they cannot see.
3. Wait for **Levi UP**. A Hard mid on the canal stays there for an ordinary peel. Drop the ally to about **22%** within about **48** studs of that mid and they leave the pit for the fight, then go back to Levi when the ally is safe. Easy and Normal never turn for this.

Smoke (not a Studio substitute): `python3 tools/test-jungle-bot.py /path/to/luau`.

### Studio live-pass (Hard siege)

Hard bots only. Easy and Normal still wait outside a post. Tower HP and gold are unchanged. This pass still does not goal the yarn core — that walk is Hard core, and only once the core reads (OPEN). Control Yarn, peel, and item use stay as they are. The jungle smoke checks which post they join and what still outranks it. It does not move a character.

1. **Hard** Practice. Shove a wave so at least two allied kittens are under an enemy outer scratching post. The Hard bot in that lane should walk in with the wave and scratch the post, not stand back last-hitting. If Red mid is already nearby, they leave a full-HP camp and join that post too.
2. Clear the kittens so the post is empty. The bot should not stay under it. A full-HP post with nobody tanking is left alone. If the post is already hurt and one kitten is still under it, they may keep hitting it. An open lantern stall (its outer post already down) follows the same rule.
3. Scratch a Hard bot low, or wait for **Levi UP**, or hurt a camp they are already on, while a wave sits on a post. Low-HP flee, a low ally peel, that hurt camp, and Canal Levi still win over the siege. A critical peel still pulls them off Levi.
4. **Easy** and **Normal** do not take this walk.

Smoke (not a Studio substitute): `python3 tools/test-jungle-bot.py /path/to/luau`.

### Studio live-pass (Hard finish)

Hard bots only. Easy and Normal keep last-hitting a healthy wave and do not leave a full-HP camp for a wounded cat beyond face range. No new damage. Stall Fang, peel, and siege stay as they are. The jungle smoke checks the HP bands and what still outranks the chase. It does not move a character.

1. **Hard** Practice. Wound a Red bot to about **40%** HP or lower in the open, where another Red bot can see them (not inside a brush that bot is outside) and within about **64** studs. That second bot should leave a full-HP camp, or stop last-hitting a healthy wave, and walk in to scratch the wounded cat. A cat at about **28%** can pull them from farther, out to about **112** studs. Above those lines they keep farming.
2. If that chase would step under your post with nobody tanking it, they stay back. Low-HP flee, a low ally peel, Canal Levi, a camp they already hurt, and a wave that is already taking a post still win. A cat already in their face is still the fight they were going to take.
3. **Easy** and **Normal** do not take this chase.

Smoke (not a Studio substitute): `python3 tools/test-jungle-bot.py /path/to/luau`.

### Studio live-pass (Hard core)

Hard bots only. Easy and Normal still do not walk the yarn core. Core HP and gold are unchanged. Outer posts and lantern stalls stay on the Hard siege pass — that pass still never goals the core. The jungle and structure smokes check the gate and what still outranks the walk. They do not move a character.

1. **Hard** Practice. Knock down every enemy scratching post and lantern stall so the yarn core billboard reads **(OPEN)**. Leave the stalls up and it should stay **(gated)** — Hard bots should not hit it. With the stalls down, shove a wave so at least two allied kittens are under the core and there are more of yours than theirs. The Hard bot in that lane, and Red mid if they are on a full-HP camp, should walk in with the wave and scratch the core.
2. Pull that wave off a full-HP core. They should not solo-walk it from the lane or the fountain. If the core is already at about half HP, or two allies are already scratching it, they may join without the kittens.
3. Scratch a Hard bot low, wound a cat in their face, wait for **Levi UP**, or hurt a camp they are already on, while the core is open with a wave. Low-HP flee, a low ally peel, that face-range cat, Canal Levi, and the hurt camp still win. A wounded cat farther out loses to the open core.
4. **Easy** and **Normal** do not take this walk.

Smoke (not a Studio substitute): `python3 tools/test-jungle-bot.py /path/to/luau` and `python3 tools/test-structure.py /path/to/luau`.

### Studio live-pass (Hard fountain)

Hard chase only. Fountain regen, fountain damage, and human movement stay as they are. The pad is the existing regen disk: radius **32** centered on the fountain spawn (x = ±176). The same disk shop and fountain regen use. The yarn core is not that pad.

1. **Hard** Practice. Wound a Red bot to about **40%** HP or lower and walk them onto the **red fountain pad**. A Hard ally who can see them should come up to that pad and **stop at the rim**. They should not follow the wounded cat onto the pad. After they stop, they go back to the lane, a camp, or a siege.
2. Leave that wounded bot just **outside** the rim, in the open, still inside the finish range (about **64** studs at 40%, or about **112** at 28%). The same ally should still walk in and scratch them. Peel, Canal Levi, a hurt camp, and a wave already on a post still win over that chase.
3. Knock every enemy post and lantern stall down so the yarn core reads **(OPEN)**, and shove a winning wave under it. A Hard bot should still walk in with that wave and scratch the core. An empty full core is still not a solo walk from the fountain.
4. Scratch a Hard bot low near your own fountain. They still recall there, shop, and regen. **Easy** and **Normal** also refuse a chase step onto the enemy pad. They still do not take the Hard finish.
...
5. **Queue together:** only the leader's **Queue party** enters every party member who is free in the hub. A member who hits **Leader queues** gets a clear no and does not solo-queue. Leader **Cancel queue** pulls the whole party and tells them. A member can leave the queue alone; the leader is told. If the leader is already queued when someone accepts, that cat joins the queue too. After a live fight (or a remake of one), the end-screen **Queue** chip leaves the post-match seat and calls that same join. A leader requeues the party. A member gets the same "they start the queue" toast and is not solo-queued. Practice end screens do not show **Queue**.
6. **Practice:** leader **Practice with party** starts one in-place Practice. Party humans share **Blue**. Blue stays at `QueueFillTo` (3), so ally bots are only the seats humans did not take (`PracticeAllies = 2` is the solo cap, never a negative count). Solo is 1 human + 2 ally bots vs 3 Red. Two humans are 2 humans + 1 ally bot vs 3 Red. Three Blue humans get 0 ally bots and still 3 Red (`PracticeEnemies = 3`). A party bigger than 3 is those humans vs 3 Red, still with no ally bots. Allies use the same difficulty as Red. They lock a cat only after every human on Blue has locked. Round-robin lanes: two allies take Top then Mid (Bot stays open); one ally takes Top only. **Again · Easy/Normal/Hard** on a Practice end screen (including a Practice remake) restarts Practice at that difficulty and brings the same party. `MatchPlaceId` can stay `0`. Practice never reserves a server. Queue fill is still `QueueFillTo` (3 per side) and is not a second copy of this count. Smoke: `python3 tools/test-practice-roster.py /path/to/luau`. `Config.Bots` itself is not asserted there (server Config is outside the shared harness).
...
3. Solo Practice: you are Blue with two ally bots, Red is three bots. If voice is actually up, the pill stays Ready and says bots are silent / no voice teammates. It should not look like a broken error. Ally bots are not teammates you can hear. There is nobody on the other side to hear you.
...
| **Tab** (hold) | Scoreboard: Blue/Red sections, YOU highlight, KDA / CS / gold / level / short items. Dead dimmed. Bots tagged. Non-modal. A teammate who has been idle **90s** gets a small **AFK** chip |
| **AFK** | Alive in a fight with no move, AA, cast, ping, shop, or recall for **45s**: local toast **Still there?**. At **90s** teammates see the chip and a quiet `{name} is away.` line. Any of those inputs clears it. No kick, no surrender, no bot |
| **F7** / **Remake** | Remake vote while the clock is **under 3:00**. Chip sits just left of **Fold**. Same living-human majority. 30s window, then **60s** cooldown. Practice can always start inside the window. A real fight needs a teammate who never connected or already left. Pass is a remake: no winner. Bots never vote |
...
2. **Cat Rift Practice.** After draft, open **?**. Title **Cat Rift**. Scan **Movement · Combat · Vision · Shop · Social · Objectives**. Confirm rows for move / AA / QWER / B / F / 4 / 5 / 6 / 7 (via item rows) / G / T / Tab / **AFK** / **F7** / **F8** / V / Mute, fog + ghosts, death recap, purse/CS, Levi, posts → stalls → core, Invite, Kitty Caster, **End**, and Tips → Live (not a card). The Practice row reads solo + 2 Blue bots vs 3 Red, and a party caps Blue at 3. **End** says Practice shows **Again ·** the difficulty, and a live fight shows **Queue**. Kit shorthand still sits under QWER. **AFK** says 45s **Still there?** and a 90s teammate tag, cleared by move / AA / cast / ping / shop / recall. **F7** says remake under 3:00. **F8** says surrender after 8:00, living humans, bots never vote.
...
2. At **8:00** the chip turns amber. **F8** or **Fold** opens a night-market panel: Yes / No counts, a ~30s timer, **Yes** and **No**. Teammates get a toast that someone called a surrender. In solo Practice you are the only living human, so that press passes immediately: the end screen is **DEFEAT** for you (**VICTORY** would be the other side), the subline says your team surrendered, and Kitty Caster still calls victory or defeat. **Again ·** the difficulty / **Back to lobby** / **Hub** are the chips.
...
### Studio live-pass (remake vote)

Cat Rift only. The Remake chip sits just left of **Fold**, under the voice pill. **F7** does the same thing. Yarn Run, Koi Pond, Yarn Party, Meme Arcade, and the hub grid do not show the chip and do not toast on F7. Surrender (**F8** / **Fold**, after **8:00**, **90s** cooldown) is unchanged.

1. **Practice**, lock in, fight. While the top-left clock is **under 3:00**, press **F7** (or **Remake**). The chip is peach. In solo Practice you are the only living human, so that press passes immediately: the end screen title is **REMAKE**, the subline is **Fight called early**, and it is not **VICTORY** or **DEFEAT**. Kitty Caster says the fight was called early and does not crown a side. **Again ·** the difficulty / **Back to lobby** / **Hub** are the chips. A remake of a live fight keeps **REMAKE** and shows **Queue** instead of Again. Bots do not open a vote on their own.
2. Start another Practice and wait until the clock reads **3:00**. The Remake chip hides. **F7** toasts that remake closes at 3:00. The match keeps going. **Fold** stays dim until **8:00**, then surrender works as before.
3. With two living humans on a side, one Yes stays open (need both). The second Yes passes and ends as a remake for everyone. With three, two Yes passes even if the third has not voted. A dead human is not in the count. A disconnected human is not in the count. Bots are not in the count and have no Yes/No.
4. Outside Practice, **F7** before anyone is missing toasts that a teammate has to have never connected (or already left). Once an allied human slot is missing, the connected living humans can start. Their Yes still needs a majority of whoever is still alive and connected — one remaining human passes alone. If that missing human rejoins, **PlayerAdded** (and character restore) sets `connected` true again, so they are no longer the missing teammate and can vote if they are alive.
5. If the 30s window runs out, or everyone living has voted and Yes is short, the panel closes and a toast says the vote failed with a **60s** cooldown. **F7** during that wait toasts the cooldown. After it, and still under **3:00**, the team can start again.
6. You cannot open **Fold** while a remake ballot is up, or **Remake** while a surrender ballot is up. Those clocks do not overlap in a normal fight (remake closes at 3:00, surrender opens at 8:00). Hub stalls never show this panel.

Smoke (not a Studio substitute): `python3 tools/test-remake.py /path/to/luau`, `python3 tools/test-surrender.py /path/to/luau`, `python3 tools/test-end-screen.py /path/to/luau`, and `python3 tools/test-announcer.py /path/to/luau`.

### Studio live-pass (AFK warning)

Cat Rift only. The **?** sheet has an **AFK** row. Yarn Run, Koi Pond, Yarn Party, Meme Arcade, and the hub grid do not toast and do not tag anyone. This does not surrender, kick, or replace anyone with a bot.

1. **Practice**, lock in, fight. Stand still. Do not move, auto-attack, cast, ping, open Pawmart, or recall. At about **45s** a toast says **Still there?**. The match keeps going. Bots do not get a toast.
2. Move (WASD is enough; an auto-attack, a cast, a ping, **B**, or **F** also counts). The idle clock resets. Hold **Tab**: your row has no **AFK** chip.
3. Stand still again until about **90s**. Hold **Tab**: your row ends with **AFK**. A human teammate also gets a quiet kill-feed line `{your name} is away.` Solo Practice has no human teammate, so you only see the chip. Enemies do not get that line or the chip.
4. Move again. Hold **Tab**: the **AFK** chip is gone.
5. Walk to your fountain and stand in the regen for a few seconds, or press **F** and let the recall channel finish. Neither one toasts early — the last move or the recall press is the stamp. Die right after you moved: the death timer does not toast you either. Hub stalls stay quiet.

Smoke (not a Studio substitute): `python3 tools/test-afk.py /path/to/luau`. Remake and surrender still: `python3 tools/test-remake.py /path/to/luau` and `python3 tools/test-surrender.py /path/to/luau`.
...
- **Traveling skillshots:** Whiskers / Bytekit / Nyan Q (and other `targeting = "line"` kits) spawn a pooled neon bolt at **72 studs/s**. Damage ticks on the server as the bolt sweeps (~0.05s); the client only follows. Not hitscan. Bolts **stop on keep walls and thick cover** (same solids as fog LoS / `MeoBlocksVision`; mid gate and thin props stay open). Ally and enemy bolts both collide; units behind the wall are not hit. Enemy bolts still hide in fog — wall collision is separate.
...
- **Ability refuse:** Cat Rift InProgress only. Q/W/E/R and **4–7** still call the server. A refused cast paints from that error: **On cooldown**, **Need mana**, or **Out of range**, plus a coral wash on that slot and a soft screen tick. Mashing waits about half a second. A landed cast does not use it. Down, unlearned, and buy-at-Pawmart keep the old toast. No silence rule and no ability "no target" rule. Smoke: `python3 tools/test-combat.py /path/to/luau`.
- **Ability cast:** Cat Rift InProgress only. Q/W/E/R and **4–7** still call the server. When that call comes back `ok = true`, the slot that cast gets a short cream wash and an amber rim, plus a screen tick quieter than the coral refuse. There is no banner. A same-slot channel cancel does not paint. Spam on cooldown still uses the coral refuse. Bolts, zones, and the scratch arc are unchanged. Hub and the other stalls stay quiet. Smoke: `python3 tools/test-combat.py /path/to/luau`.
...
- **Bots:** Normal/Hard lead with the same projectile speed (0.7s cap) and sidestep for `travel + 0.12s` so they still dodge the bolt instead of the old instant ray. **Shop:** Hard buys a role plan at the opening fountain through the same Pawmart grant humans use (`ShopService.buyAt`: fountain, gold, stack cap). Easy stays naked until one Mana Treat on a later visit. Normal buys two stat sticks on a fountain return. Hard presses Yarn Cleave when two visible enemy cats are in the 12-stud circle, and Whisker Lens only after buying it and an enemy ward is inside the lens radius. Hard buys Control Yarn (75g, two charges) after Fang or Cleave and before the lens; an unaffordable Fang or Cleave still blocks it. When a charge is ready they plant toward a river brush, the canal, or a crab approach if that point is in reach and they are not under an enemy post, one plant per charge, reusing the warded latch. Easy and Normal do not buy it. **Jungle:** Easy never clears. Normal mid takes the nearest own-side camp only after the wave is pushed, and finishes a camp it has already hurt. Hard mid follows pigeon → golem → near crab → far crab and holds that goal between thinks (the old walker used to yank them back to lane every tick, and equidistant crabs could swap). After the route is down, Hard rotates to the furthest allied wave, with an 8-second last-known bias from team vision (allies, trinkets, pinks). When **Canal Levi** is up, Hard mid may path to the canal first if they are not already holding a camp; Normal and Easy ignore it. A visible ally at or under about 40% HP with an enemy on them pulls a Hard bot off a full-HP camp, a soft hunt, or the lane shove. Levi still wins unless that ally is at or under about 22% and the fight is nearby. A hurt camp, low-HP flee, and an unsafe tower still come first. Easy and Normal do not peel. When allied kittens are already under a vulnerable enemy post or an open stall (two on a healthy post, or one if it is already hurt), a healthy Hard bot hits that structure instead of a full-HP camp. An empty post and the yarn core are not that walk. Peel, Levi, a hurt camp, and flee still come first. Easy and Normal do not siege. Camp spots and own-side uptime are map knowledge; enemy champions are not. A hidden cat is not a target. The living Levi is still fog. Smokes: `python3 tools/test-jungle-bot.py /path/to/luau`, `python3 tools/test-combat.py /path/to/luau`, `python3 tools/test-bot-shop.py /path/to/luau`, `python3 tools/test-items.py /path/to/luau`.
- **Pink vs trinket:** **4** is a stealthed team-tinted pillar. **6** (after Pawmart **Control Yarn**, 2 charges) is a magenta ball enemies can see. Pinks grant a bit more team vision, slow foes in 22 studs, and keep nearby enemy trinkets revealed. Hard jungle bots still drop one free pink. Hard bots that own Control Yarn plant a bought pink toward river brush, the canal, or a crab approach, one charge at a time, and not under an enemy post. Both kinds feed the server fog mask.
- **Pawmart night market:** **7** is Yarn Cleave (280g, 12-stud slash, 12s cooldown, visible enemies only). Stall Fang (300g) adds 36 raw damage to champion hits at or below 20% HP. Paper Charm (220g) ignores an enemy pink slow for 2.4s, then waits 20s. None of the three add flat damage, HP, mana, armor, or speed. Lens stays on **5**. Control Yarn still stacks to 2. Hooks live in `Shared.ItemKits`. Cat Rift juice (`Shared.ActiveJuice`) paints those uses: cream glance on **5**, cream arc on **7**, coral bite beside the execute number, pink puff then a soft pop for the charm window. Prices and timings unchanged. The **B** panel is UI-only polish (`Shared.ShopPaint`): Stats / Vision / Actives sections, **5** / **6** / **7** chips, purse-matched gold, fountain warn, buyable vs short vs owned tint — prices unchanged. Smoke: `python3 tools/test-items.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.
...
- **Recall:** **F** channels 7s, then the server teleports you to your fountain. While channeling, an amber/cream swirl fills under your feet. Success flashes mint and puffs at the fountain. A new move order (release and press WASD / stick, or left-click empty ground) cancels immediately, shatters the swirl, and pops **RECALL CANCELED**. Attack, attack-move, a successful cast, **S**, **F** again, and damage still cancel the same way. Camera, help, emotes, pings, and opening Pawmart do not. Being shoved more than 2.5 studs still cancels. Smoke: `python3 tools/test-recall.py /path/to/luau`. Studio live-pass under recall channel juice.
...
### Studio live-pass (last-hit gold)

Client juice only. Gold amounts, CS credit, and the purse chip stay in `FarmCredit`. A last hit is the credit with CS (`FarmCredit.lastHit`). Kill, assist, and tower gold are bounties and must not show this amber CS float — they paint on the bounty live-pass. Smoke: `python3 tools/test-farm-credit.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Last-hit a lane kitten (the killing scratch). An amber **+18g** rises at the corpse with a tiny cream **+1 CS** under it, and a soft coin tick. A separate lavender **+18 XP** may rise above that gold. The purse chip gold and CS both climb by that same last hit. No peach bounty float on that kitten.
2. Buy Yarn Cleave (**7**) and slash a wave. Near-simultaneous last hits merge into one **+Ng** (CS tick sums) instead of a carpet. The purse still gains one CS and the kitten gold for each kitten.
3. Kill a Red bot. The purse gains kill gold and CS does not move. The amber **+CS** float does not play. The peach bounty float is the next live-pass. An assist, or knocking a scratching post, also bumps gold with no CS tick.
4. Optional: clear a jungle camp. Amber **+Ng** matches that camp's gold and CS goes up by one. Canal Levi's take still shows the pit burst and the purse **+8%** line; the last-hit float is only the camp gold.
5. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Ally and enemy last hits do not float gold on your screen.

### Studio live-pass (bounty gold)

Client juice only. Kill gold stays **180**, assist gold **60**, tower / nexus gold **120**. None of them add CS. A bounty is the credit with gold and no CS (`FarmCredit.bounty`). The server emits `CombatFx` `kind = "bounty"` with that same gold. Last-hit kitten gold stays amber and on `kind = "lastHit"`. Smoke: `python3 tools/test-farm-credit.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Kill a Red bot. A peach **+180g** rises on the victim. There is no cream **+CS** line. A lavender **+80 XP** may rise above the gold. The purse gains 180 gold and CS stays put. The amber last-hit float does not play for that kill.
2. Scratch a Red bot and let an ally take the kill. You get a peach **+60g**. CS does not move. Your ally's **+180g** does not float on your screen.
3. Knock an outer scratching post, or the yarn core once it is open. A peach **+120g** rises at the structure. The fall (planks, lantern, or yarn strands) still plays. CS does not move.
4. Last-hit a lane kitten. Amber **+18g** and cream **+1 CS** still play. No peach bounty float on that kitten.
5. Two takedowns close together stay readable: the numbers merge inside the short window or stop once the screen is full, instead of covering the fight. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet.

### Studio live-pass (XP float)

Client juice only. Nearby kitten XP stays **10**, nearby camp XP **12**, last-hit kitten XP **18**, kill XP **80**, assist XP **30**. The share radius stays **42**. Level thresholds stay `40 + (level-1)*28`. `grantXp` adds that same number, then emits `CombatFx` `kind = "xp"`. Gold and CS stay on their own floats. The **LEVEL UP** banner still comes from the level edge. Smoke: `python3 tools/test-farm-credit.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Stand near a lane kitten and let an ally, a bot, or the wave last-hit it. Do not take the killing scratch yourself. A cream/lavender **+10 XP** rises at that kitten. No amber **+18g** and no cream **+CS** on your screen for that death. The amber XP bar still fills by 10.
2. Last-hit a lane kitten yourself. The amber **+18g** and cream **+1 CS** still play, with the coin tick. A lavender **+18 XP** rises above them. No peach bounty float on that kitten.
3. Optional: stand near a camp clear you did not last-hit. **+12 XP**. Last-hit the camp and the camp's own XP rises with the amber camp gold. Kill a Red bot and **+80 XP** sits above the peach **+180g**. An assist is **+30 XP** above the peach **+60g**.
4. Let a wave die around you, or Yarn Cleave (**7**) a clump. Near-simultaneous grants merge into one **+N XP**, and the screen stops adding new pops once it is full, instead of a carpet. The XP bar still gains each grant.
5. Farm until the bar tips a level. The cream/amber **LEVEL UP** banner, the foot ring, and the ability-bar pulse still play once. The XP float for that grant still says **+N XP**, not a second **LEVEL UP**.
6. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. An ally's XP does not float on your screen. Match start, wave lanterns, camp countdowns, and tower lock rings look the same.

### Studio live-pass (auto-attack scratch)

Client juice only. Auto-attack damage stays **24** plus items, range **26**, interval **1.05s**, windup **0.16s**. `tickAutoAttack` still owns the swing. The windup sends `CombatFx` `kind = "aa"` with `duration` set to that windup (`ScratchJuice.beat` reads windup). The landing emit is the same `aa` with no duration, plus `targetUserId` when the target is a champion. Damage numbers stay on `kind = "float"`. Yarn Cleave on **7** keeps the wider arc. Smoke: `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Click a Red bot in scratch range. A short cream claw reaches toward them, then a narrow cream scratch arc plays when the hit lands. The damage number is the same cream/coral float as before. The arc does not add a second number. Range and attack speed feel the same.
2. Let a Red bot scratch you. A tiny cream tick marks the hit on you. The damage float is still the only number.
3. Buy **Yarn Cleave** (280g) and press **7** next to that bot. The cleave arc is wider and reaches farther than the scratch. The next auto-attack is still the short claw, then the narrow arc.
4. Cast a traveling line (**Q** on a cat that fires one). The bolt still travels. It does not look like the scratch arc.
5. An ally scratching a bot you can see gets a fainter claw and arc. Walk so a bot is scratching inside brush you are not in: you do not get an enemy arc from that brush. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. XP floats, tower lock rings, wave lanterns, and the Fight! cue look the same.

### Studio live-pass (structure aggro)

Client juice only. Who the post shoots, how far, and how hard stay in `TowerService`. The server writes the champion `pickTarget` already chose onto `MeoLockUserId`, including the moment before the next bolt. A kitten target clears that id. The bolt is still `towerShot`; the coral flash is paint on that same event. Smoke: `python3 tools/test-structure.py /path/to/luau` and `python3 tools/test-combat.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Walk mid under the Red outer scratching post with no lane wave (before the first kittens, or after a wave has gone past).
2. While the post has you, a soft coral ring sits at its foot and a thin coral ring sits under you. Step out of range and both rings clear. Step back in before the next bolt: the rings return while it is still cooling, because that is who it will shoot.
3. When the bolt lands, a short coral flash rises on you. The beam stays the team color. The damage float is the same number as before — there is no second hit.
4. Scratch so the post locks you, then let a closer kitten walk into range while that aggro window is still up. The post keeps you, same as before, and the rings stay. When the window ends and the kitten is closer, the rings clear and the bolt goes to the kitten.
5. Optional: an ally under a post you can see gets a fainter ring on that post, and no foot ring on you. If that post is in fog, you do not see their cue. A Red bot under your own post does not get a marker.
6. Knock the post down and dive the lantern stall with no wave. The stall bolt stays lantern gold, and a warm glare sits on the stall with the coral ring. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Bounty floats, brush, Levi's rim, and structure falls look the same.

### Studio live-pass (camp respawn)

Client juice only. Pigeon **32s**, yarn golem **45s**, and river crab **50s** stay in `JungleService`. Gold is unchanged. Canal Levi still uses the match-clock line, not this chip. Smoke: `python3 tools/test-jungle-bot.py /path/to/luau`, `python3 tools/test-vision.py /path/to/luau`, and `python3 tools/test-minimap.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Clear the NW pigeon pack (your side). A small cream/amber countdown sits at the empty pit, about **32s**, and the minimap shows a faint square there with the same seconds. The living amber camp pip is gone until it returns.
2. Walk back toward the lane so the pit leaves your vision. The countdown and the minimap mark stay, because you already explored that ground. They do not follow you.
3. Leave the enemy jungle unexplored. If a Red bot clears the SE pigeon before you have seen that pit, no countdown and no minimap mark appear there. Walk onto that empty pit later: the chip shows once the ground is lit. An unexplored camp stays hidden.
4. Wait out the pigeon you cleared. At **0s** a brief soft cream/amber puff plays at the pit, the countdown leaves, and the pack is back under the normal camp rules (body and minimap pip only while you can see it). A golem or crab uses the same chip with its own existing timer.
5. Canal Levi still counts on the top-left clock (**Levi UP** / **Levi 0:47** / **Levi taken**). This pit chip does not appear on the canal. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Tower lock rings, bounty floats, and the brush wash look the same.

### Studio live-pass (brush juice)

Client juice only. The pockets are the same ones fog already uses (`VisionLogic.Brushes` / `brushAt`, painted by `MapBuilder`). Walking in does not change who can see you. Smoke: `python3 tools/test-vision.py /path/to/luau`.

1. Hub → **Cat Rift → Practice**. Walk into a jungle brush pocket (NW or NE by the blue camps, or a river-crab pocket). A soft mint leaf wash and a quiet foot hush play, and the screen darkens a little. While you stay inside, a very light green tint holds under you — not a particle storm.
2. You stay hidden to fogged enemies. A Red bot outside that pocket should drop you off their view the same way as before: you vanish from anyone who has not entered that pocket, and you do not see a marker on a cat inside a pocket you have not entered. Last-seen ghosts still freeze and fade where someone left vision.
3. Walk back out onto the lane. A soft cream fade clears the tint. Open lane ground does not play the wash. Step in and out quickly: the burst does not stutter.
4. Optional: an ally who is already visible and steps into brush with you may get a very faint hush. An enemy in brush does not, even if you share the pocket.
5. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Last-hit gold floats, the Levi rim, and structure falls behave as before.

### Studio live-pass (skillshot wall collision)

Combat feel. Same walls as fog LoS (keep walls + market drums / `MeoBlocksVision`). Smoke covers the clip math only.

1. Hub → Cat Rift → Practice. Draft **Professor Whiskers** (or Bytekit / Nyan). Stand outside the Blue keep and hold **Q** aimed **through** a keep wall (not the mid gate). Release: the bolt travels, **bursts on the wall**, and does not continue into fountain or the far side.
2. Fire **Q** through the **mid-lane gate**: the bolt still travels the full line (gate stays open).
3. Aim a bolt into a **market drum** (between mid and a side lane): it stops on the drum. Aim beside the drum through the gap: it continues.
4. Fire past a fountain pillar / thin prop / brush pocket: the bolt is **not** stopped (vision already skips those).
5. Let a Red bot fire a line skillshot into a wall you can see: their bolt also stops. Walk so an enemy bolt is in fog: it still hides while traveling; wall collision does not change fog show/hide.
...
### Studio live-pass (recall channel juice)

Client juice only. Server still owns the 7s channel, cancel-on-order, and the fountain teleport. Smoke: `python3 tools/test-recall.py /path/to/luau` (RecallLogic + RecallJuice).

1. Hub → Cat Rift → Practice. Walk off the fountain and press **F**. An amber/cream foot swirl fills over the 7 seconds, the ability-bar channel still counts down, the cat gets a soft wash, and a quiet tick sits under the recall loop. Stand still.
2. Let it finish. A brief mint screen flash, you appear at your fountain, and a mint puff marks the pad. The swirl and the channel bar are gone.
3. Press **F** again and tap a **new** move (or click empty ground). The swirl shatters coral and a short **RECALL CANCELED** banner pops. The bar clears. A direction you were already holding when you pressed **F** still does not cancel until you release and press again.
4. Start another recall and cast, press **7**, or take a hit. Same cancel juice. **B**, **T**, **G**, **H**, and **V** do not cancel.
5. Optional: a teammate recalling shows a faint ring (even through fog). A Red recall stays hidden until you have vision, then only a faint ring — no mint puff in fog.
6. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Fountain regen, walls, nameplates, kits, and Pawmart are unchanged. The channel is still 7 seconds.

### Studio live-pass (Control Yarn juice)

Client juice only. Server still owns place range, the 90s / 60s lifetimes, stack caps, and who is in the vision ward list. Smoke: `python3 tools/test-ward-juice.py /path/to/luau`.

1. Hub → Cat Rift → Practice. At the fountain press **B**, buy **Control Yarn** (75g, two charges). Walk off the pad and press **6** on open ground. A soft cream/amber puff lands, a cream pole with an amber lantern and a bobbing yarn ball sits on the magenta ball, and a short **Control Yarn planted** float rises. The ability bar charge drops by one. Duration is still about 90 seconds.
2. Press **6** again far away (or with no charges). The existing refuse line toasts (**Ward out of range.** or **Buy Control Yarn at Pawmart (B). Press 6 to plant.**). No puff.
3. Plant a trinket with **4**. Same cream/amber bob, float says **Ward planted**. One trinket per cat; the next **4** replaces it (pop on the old one, puff on the new).
4. In the last stretch before it expires, the lantern pulses and fades. When it goes, a clear amber pop plays and the minimap dot disappears. The minimap dot was already there and only gets fainter near the end — it does not appear for a ward you cannot see.
5. Optional two clients: your ally sees the cream/amber lantern. A Red cat sees your pink (it was already visible) with a dimmer pink silhouette, including in fog. They do **not** see your trinket, its puff, or an expire pop unless they walk onto it, lens it (**5**), or a pink reveals it. No new dark-ground leak.
6. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Recall, fountain regen, walls, kits, and Pawmart prices are unchanged.

### Studio live-pass (death respawn juice)

Client juice only. Server still owns the respawn seconds, the fountain teleport, the full heal, death-recap lines, and gold loss. Die in Practice.

1. Hub → Cat Rift → Practice. Let a Red bot drop you. A cream/amber **Respawn** chip sits center-top (under the banner, above the recap card) and counts down in seconds (`6s` at level 1). A soft ink wash dims the screen. The death recap card still lists the scratches. The ability-bar line still says you are down.
2. In the last **3** seconds the chip pulses amber. It reaches **0s** only as the fountain return happens. **✕** still dismisses the card; **Recap** still brings it back while you are down. The chip stays up either way.
3. You appear at your fountain. A brief mint puff sits on the pad and your cat gets a short mint silhouette wash. No loud heal flash and no Heal ping on that return (fountain regen, if you are missing HP later, is the calm ring — not a second puff spam). The chip and the wash clear. The recap card and **Recap** chip clear too.
4. Optional: an ally bot coming back may show a smaller mint puff and a faint **back** on their fountain. A Red respawn stays quiet unless you can already see their fountain, and even then it is only a faint ring — no **back** label through fog.
5. Hub, Yarn Run, Koi Pond, Yarn Party, and Meme Arcade stay quiet. Recall, ward juice, fountain regen rates, kits, Pawmart, and the death recap rules are unchanged.

Smoke (helpers only, not a Studio substitute): `python3 tools/test-combat.py /path/to/luau` and `python3 tools/test-death-recap.py /path/to/luau`.
...
- **6** magenta pink is visible to Red even in fog, and still grants your team a vision bubble + slow + trinket reveal. Pink vision still respects walls and thick cover (a drum between the pink and a cell stays dark). Ally wards wear a cream/amber lantern; a revealed enemy ward wears a dimmer pink one. That paint follows the same show/hide as the ward part — it does not light fog or add a minimap dot.
- Stand in a labeled brush pocket: you should drop off the enemy minimap until they enter. You can still see the lane from inside. Entering plays a soft mint wash and leaving plays a cream fade (local juice only — it does not light the pocket or mark an enemy). **LMB / attack-move (X)** must **not** lock an unseen brush target. Attack-move toward a last-seen brush pocket walks **into** that last-known tile (never the hidden live body).
