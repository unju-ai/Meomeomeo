...
Canal Levi (same day, later slice): one river epic, not a dragon / baron / herald set. It wakes at **3:00**, 1100 HP, in the canal between mid and the south crab. The last hit's team keeps **+8% auto-attack and ability damage** for the rest of the match (the purse chip grows a line; Kitty Caster calls it). It returns **once**, about 90 seconds later, then stays down. A second take on the same side does not stack. The other side can earn its own buff on the return. Fog still hides the living body. The minimap shows a pit marker while it is up in fog, or while it is waiting to return, and a larger body dot only in vision. Hard mid may path to it when it is up and they are not already on a camp. Normal and Easy ignore it. Verified locally: Luau compiled 135 sources; `tools/test-jungle-bot.py` and `tools/test-combat.py` passed (3:00 wake, one 90s return, non-stacking +8%, pit vs body, Hard detour only). `tools/test-announcer.py` and `tools/test-old-tom.py` still passed. The other mode smokes still passed. Rojo 7.4.4 built the place. Studio still has to fight it (the live-pass below).
...
4. After those four camps are down (or you steal them), and **before Canal Levi is up**, Red mid should **rotate** into the lane where your minions are furthest forward, not pace the river. A pink or a recent sighting in another lane can pull that rotate. They should not path to a brush you vanished into and never re-enter. Once Canal Levi is alive, Hard mid may leave that rotate for the canal (see the Canal Levi pass). They still finish a camp they already started.
...
The Luau smokes check the 3:00 wake, the one 90-second return, the non-stacking +8% buff, the pit-versus-body minimap rule, and that only Hard mid detours. They do not spawn the beast. Fight it in Studio Play. The six small camps are unchanged.

1. **Practice**, any difficulty. The copper **Canal Levi** pad sits in the river between mid and the south crab (`x = 0`, `z = 24`). The beast is not there at 0:00. Camps still pop about 8 seconds in. Levi does not.
2. Play until about **3:00**. Levi wakes: a large copper ball, about **1100** HP, nameplate **Canal Levi**. It does not sit on the mid lane. It leashes at about **52** studs (a small camp still resets around 38).
3. With no vision on it, the minimap shows a **square pit** at that canal spot, not the body if someone has dragged it. Walk a ward or a cat into vision: the living body appears (a larger dot), and the world model is visible. Step back out: the body hides again. Fog rules are the same as any camp.
4. Last-hit it (the killing scratch). Your purse chip gains **Canal Levi · +8% damage**. Kitty Caster says Canal Levi is down and names your side. Autos and abilities hit about 8% harder for the **rest of the match**. Yarn Cleave, towers, and kittens do not get that amp. A second take does not add another 8%. Gold is **140** and CS goes up by 1. The meme tape does not move.
5. About **90 seconds** later it wakes once more. If the other team last-hits that one, they get their own chip and their own caster line. Your buff stays. After that second death it does not come back, and the pit marker leaves.
6. **Hard** practice, after 3:00: Red mid may walk to the canal when Levi is up, unless they are already clearing a camp (they finish that first). Red top and bot stay in lane. **Normal** mid does not leave for Levi. **Easy** never does. Hub, Closet, voice, and kits are unchanged.
...
| **Purse** | Above the ability bar during a Cat Rift match: your gold, CS, level, and whether Yarn Cleave (280g) fits. Same numbers as Tab. Hidden on the hub and the other stalls. After your side takes **Canal Levi**, the chip adds **+8% damage** |
| **Canal Levi** | One river epic. Wakes about 3:00 in the canal between mid and the south crab. Last hit's team keeps +8% auto and ability damage for the match. Returns once after about 90 seconds |
